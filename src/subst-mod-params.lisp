...
(defvar *subst-mod-params-id* nil)
...
(defun instantiated-copy (modinst)
  (assert (typep modinst '(or string modname)))
...
(defmethod copy-formal ((fml formal-subtype-decl) bindings)
  (let ((ntype-expr (substit (type-expr fml) bindings)))
    (assert (type-expr? ntype-expr))
    (copy fml :place nil :newline-comment nil
	  :type-expr ntype-expr)))
...
(defmethod subst-theory-params ((theory module) (alist cons))
  ...
  (multiple-value-bind (nformals nacts)
...
(defun subst-mod-params (obj modinst &optional theory decl new-id)
  (when obj
    ...
    (assert (or (null decl)
		(every #'(lambda (x)
		       ...
		       (freevars obj))))
    (when (and (recursive-type? obj)
	       (null new-id))
      (error "Need to include :new-id when a (co)datatype is involved"))
    (when (and (module? obj)
	       (generated-by obj)
	       (recursive-type? (get-theory (generated-by obj))))
      (error "Use ~a instead of ~a, and then use the adt-{map-|reduce-)theory slot ~
              of the returned instance" (generated-by obj) (id obj)))
    (let* ((*substit-dont-simplify* t)
	   (*subst-mod-params-id* (if (stringp new-id) (intern new-id :pvs) new-id))
	   (*subst-mod-params-theory* (or theory
		...
		(subst-mod-params* obj modinst bindings)
	      ...
	      (assert (or (null decl)
			  (every #'(lambda (x)
				 ...
				 (freevars obj))))
	      #+pvsdebug
	      (assert (or (print-type-expr? obj)
			  (not (print-type-expr? nobj))))
...
(defun make-subst-mod-params-bindings (modinst formals actuals mappings bindings)
  (cond ((null formals)
         #+pvsdebug (assert (check-smp-bindings bindings))
         (let ((mbindings (make-subst-mod-params-map-bindings
			   modinst mappings bindings)))
...
(defun get-interpreted-mapping (theory interpretation theory-name)
  (let* ((*subst-mod-params-theory* theory)
...
(defun make-subst-mod-params-map-bindings (modinst mappings bindings)
  (if (null mappings)
      (if (same-id modinst *subst-mod-params-theory*)
	  bindings
	  (let ((nbindings (mapcar #'(lambda (bdg)
				       (let ((tbdg (assq (car bdg) bindings)))
					 (cons (cdr bdg) (cdr tbdg))))
			     (theory-mappings *subst-mod-params-theory*))))
	    ;; nbindings is the composition of theory-mappinsg and bindings
	    (append bindings nbindings)))
...
(defmethod make-subst-mod-params-map-bindings* ((lhs-decl theory-reference) rhs bindings)
  (let* ((rdecl (declaration (expr rhs)))
	 (rthname (if (theory-abbreviation-decl? rdecl)
	 ...
	 (inv-mappings
	  (mapcar #'(lambda (m)
		      (if (actual? (cdr m))
                          (cons (declaration (actual-value (cdr m))) (car m))
			  (cons (cdr m) (car m))))
...
(defmethod subst-mod-params* :around (obj modinst bindings)
  #+aaaa (declare (type hash-table *subst-mod-params-cache* *subst-mod-params-eq-cache*))
  (when obj
    #+badassert
    (assert (or (module? obj)
		...
		(not (some #'(lambda (bdg)
			   ...
			   bindings))))
    (let ((fixed-type (unless (mappings modinst)
			(car (member obj (list *boolean* *number* *number_field*
      ...
      (cond (fixed-type
	    ...
	    (t
	     (let ((hobj (or (gethash obj *subst-mod-params-eq-cache*)
		   ...
		   (let ((nobj
			  (if (and (null *subst-mod-params-map-bindings*)
				   ...
				   ;; 	      bindings))
				   (not (typep *subst-mod-params-declaration*
					       'importing-entity))
			 ...
			 (let ((nnobj (subst-mod-params-print-type obj nobj
								   modinst bindings)))
			   (unless (eq nnobj nobj)
			     (setq nobj nnobj))
			   (assert (not (print-type-expr? nobj)))))
...
(defun subst-mod-params-print-type (obj nobj modinst bindings)
  ;; obj has a print-type; obj and nobj may be eq, but the print-type
  ;; substitution might be different.
  (let* ((ptype (subst-mod-params-print-type* (print-type obj) modinst bindings))
	 (nnobj (cond ((eq nobj obj)
		       ;; It's possible for the print-type to have substitutions, even
		       ;; though the type doesn't
		       (if (eq ptype (print-type obj))
			   obj
			   (copy obj :print-type ptype)))
		      ((not (eq (print-type nobj) ptype))
		       (copy nobj :print-type ptype))
		      ((occurs-in nobj ptype)
		       nobj)
		      (t (setf (print-type nobj) ptype)
			 nobj))))
    (assert (or (and (or (null (print-type obj))
...
(defmethod subst-mod-params-print-type* ((ptype print-type-name) modinst bindings)
  "subst-mod-params-print-type* substitutes for the print-type."
  (assert (resolution ptype))
  (assert (null (type (resolution ptype))))
  (let* ((pdecl (declaration ptype))
	 (bdg (cdr (assq pdecl bindings))))
    (cond ((typep bdg '(or actual decl-formal-type))
...
(defmethod subst-mod-params-print-type* ((pt print-type-application) modinst bindings)
  ...
  #+pvsdebug
  (assert (every #'(lambda (p) (fully-typed? p)) (parameters pt)))
  (let ((ptype (subst-mod-params* (type pt) modinst bindings))
	(parms (subst-mod-params* (parameters pt) modinst bindings)))
    (lcopy pt :type ptype :parameters parms)))

(defmethod subst-mod-params-print-type* ((pt print-expr-as-type) modinst bindings)
  (assert (null (supertype pt)))
  (assert (null (predicate pt)))
  (assert (null (print-type pt)))
  (let* ((nex (subst-mod-params* (expr pt) modinst bindings))
	 (npt (lcopy pt :expr nex)))
    #+pvsdebug
    (assert (fully-typed? nex))
    #+pvsdebug
    (assert (fully-instantiated? nex))
    #+pvsdebug
    (assert (fully-instantiated? npt))
    npt))
...
(defun subst-mod-params-decls (decls modinst bindings nth &optional ndecls)
  (cond ((null decls)
	 (assert (every #'cdr bindings) () "subst-mod-params-decls: bad bindings")
	...
	(t (multiple-value-bind (ndfmls ndacts dfbindings)
	       ;; ndfmls are new decl-formals
	       ;; ndacts are the corresponding new actuals
	       ;; dfbindings is just (pairlis ndfmls ndacts)
	       ;;   Note that dfbindings should only be used for (car decls);
	       ;;   don't pass it recursively
	       (subst-mod-params-formal-decls
		(decl-formals (car decls)) (car decls) modinst nil)
	     (assert (every #'cdr dfbindings) () "subst-mod-params-decls - bad dfbindings")
	     (let* ((*subst-mod-free-params* (append (decl-formals (car decls))
						     *subst-mod-free-params*))
		    (decl (car decls))
		    (ndecl (copy decl
			     :decl-formals ndfmls
			     :generated-by decl :module nth :place nil
			     :pvs-sxhash-value nil :ir-type-value nil
			     :newline-comment nil :semi t))
		    (bpair (assoc decl bindings :key #'declaration))
		    (bdecl (car bpair))
		    (lmodinst (lcopy modinst :dactuals ndacts))
		    ;; rbindings comes from mappings with decl-formals
		    ;;   Should not be included in recursive call
		    (rbindings (when (mapping-lhs? bdecl)
				 (pairlis (decl-formals bdecl) ndacts)))
		    (bval (subst-mod-params* (cdr bpair) lmodinst rbindings))
		    ;; nbindings is for recursive decls, and for the remaining decls
		    (nbindings (if (or bpair
				       (typep decl '(not (or var-decl const-decl type-decl))))
				   bindings
		 ...
		 (if (actual? bval)
		     (cond ((type-value bval)
			   ...
			   ((const-decl? ndecl)
			    (unless (definition ndecl)
			      (let* ((ndef (make!-application-for-formals
					    (expr bval) (formals ndecl)))
				     (sndef (subst-mod-params* ndef lmodinst ndecl-bindings)))
				(setf (definition ndecl) sndef))))
			   ((typep ndecl '(or theory-reference))
			    (assert (theory-name-expr? (expr bval)))
			    (if (mod-decl? ndecl)
				(setf (modname ndecl) (expr bval))
		     ...
		     (break "What is bval?"))
		 (when (mapping-rhs-rename? bval)
		   (let* ((idstr (string (id ndecl)))
			  (mid (id (expr bval)))
			  (last-pos (position #\. idstr :from-end t))
			  (nidstr (when last-pos
				    (concatenate 'string
				      (subseq idstr 0 (1+ last-pos)) (string mid))))
			  (nid (if nidstr (intern nidstr :pvs) mid)))
		     (setf (id ndecl) nid))))
		 ...
		 (subst-mod-params* ndecl lmodinst ndecl-bindings))
	       #+badassert
	       (assert (not (some #'(lambda (bdg)
				      (and (decl-formal? (car bdg))
					   (not (memq (car bdg) (decl-formals decl)))))
	       ...
	       (let* ((tcc-cmts (when (tcc? ndecl)
		      ...
		      (in-tcc-cmts (some #'(lambda (cmt) (eq (car decls) (caddr cmt)))
					 tcc-cmts))
		      (rem-decls (if (inline-recursive-type? ndecl)
				     (remove-if #'(lambda (d) (eq (generated-by d) (id decl)))
				       (cdr decls))
				     (cdr decls)))
		      (new-decls (cond ((inline-recursive-type? ndecl)
					(append (generated ndecl) (cons ndecl ndecls)))
				       (in-tcc-cmts ndecls)
				       (t (cons ndecl ndecls)))))
		 ;; (when (inline-recursive-type? ndecl)
		 ;;   (break "subst-mod-params-decls inline-recursive-type"))
		 ;; every and some are missing, since they check for positive types
		 ;;   (assert (= (length (generated ndecl)) (length (generated decl)))))
		 (subst-mod-params-decls rem-decls modinst nbindings nth
					 new-decls)))))))
...
(defmethod subst-mod-params* ((nadt inline-recursive-type) modinst bindings)
  "Called from subst-mod-params-decls, which takes care of any decl-formals,
reflected in the modinst and bindings. nrectype is a copy (of generated-by)
with decl-formals and module already updated, the rest are updated here."
  (assert (generated-by nadt))
  (let* ((*adt* nadt)
	 (*subst-mod-params-id* (id nadt))
	 ;; (th (declaration modinst))
	 (acts (actuals modinst))
	 ;; (dacts (dactuals modinst))
	 ;; (tmi (module-instance (adt-type-name nadt)))
	 (adt-modinst (let* ((nmi (mk-modname (id (adt-theory nadt)) acts))
			     (res (make-resolution (adt-theory nadt) nmi)))
			(setf (resolutions nmi) (list res))
			nmi))
	 (nadt-tname (subst-mod-params* (adt-type-name nadt) adt-modinst bindings))
	 (ntype-name (mk-type-name *subst-mod-params-id*))
	 (fbindings (acons nadt-tname ntype-name bindings))
	 (nsubtypes (subst-mod-params* (subtypes nadt) modinst fbindings))
	 (nadt-imps (subst-mod-params-adt-importings nadt modinst fbindings)))
    (multiple-value-bind (nass abindings)
	(subst-mod-params-decls (assuming nadt) modinst fbindings (module nadt))
      (let ((nconstrs (subst-mod-params-constructors nadt modinst abindings)))
	(setf (importings nadt) nadt-imps
	      (assuming nadt) nass
	      (constructors nadt) nconstrs
	      (adt-type-name nadt) nil
	      (generated nadt) nil
	      (positive-types nadt) nil)
	(when nsubtypes
	  (setf (subtypes nadt) nsubtypes)))
      (let ((*adt* nadt)
	    (*adt-vars* (make-hash-table :test #'eq))
	    (*generating-adt* nadt))
	;; (break "subst-mod-params* inline-recursive-type")
	(generate-inline-adt nadt))))
  nadt)

(defmethod subst-mod-params* ((adt recursive-type) modinst bindings)
  "Creates the instance of the adt, then calls typecheck to invoke datatype.lisp functions.
e.g., (subst-mod-params-module (get-theory \"lift\") lift[nat] '|lift_nat|) creates the
datatype on the right, then typechecks it.
lift[T: TYPE]: DATATYPE BEGIN  | lift_nat: DATATYPE BEGIN
  bottom: bottom?              |  bottom: bottom?
  up(down: T): up?             |  up(down: nat): up?
 END lift                      | END lift_nat"
  (assert (not (inline-recursive-type? adt)))
  (assert *subst-mod-params-id* () "*subst-mod-params-id* not set?")
  (when (get-theory *subst-mod-params-id*)
    (pvs-error "Id ~a clashes with existing theory" *subst-mod-params-id*))
  ;; subtypes
  ;; importings
  ;; assuming
  ;; constructors
  (let* (;; (fmls (formals-sans-usings adt))
	 (afmls (formals-sans-usings (adt-theory adt)))
	 (acts (actuals modinst))
	 (*subst-mod-free-params* (append afmls *subst-mod-free-params*))
	 (nbindings (progn (mapc #'(lambda (afml act)
				     (assert (not (assq afml bindings)))
				     (push (cons afml act) bindings))
				 afmls acts)
			   bindings))
	 (tmi (module-instance (adt-type-name adt)))
	 (adt-modinst (let* ((nmi (mk-modname (id tmi) acts))
			     (res (make-resolution (adt-theory adt) nmi)))
			(setf (resolutions nmi) (list res))
			nmi))
...
(defun smp-insert-missing-adt-decls (adt nadt modinst bindings)
  (let ((adt-decls (theory (adt-theory adt)))
	(nadt-decls (theory (adt-theory nadt)))
	(alen (length (string (id adt))))
	(nlen (length (string (id nadt)))))
    (smp-insert-missing-adt-decls* adt-decls nadt-decls
				   adt nadt alen nlen modinst bindings nil)))
...
(defun smp-insert-missing-adt-decls* (adt-decls nadt-decls
  ...
  (if (null adt-decls)
	...
	(if (or (eq (id adecl) (id ndecl))
		(and (eq (id adecl) (id adt))
		     (eq (id ndecl) (id nadt)))
		(let ((astr (string (id adecl)))
		      (nstr (string (id ndecl))))
		  (and (> (length astr) alen)
		       (> (length nstr) nlen)
...
(defun subst-mod-params-adt-importings (adt modinst fbindings)
  ...
  (let ((all-imps (append (remove-if #'formal-decl? (formals adt))
			  (importings adt))))
    (mapcar #'(lambda (imp)
		;; Need to substitute fbindings in imps
		(subst-for-formals imp fbindings))
      all-imps)))

(defun subst-mod-params-constructors (adt modinst fbindings)
  ;; Need to substitute for fbindings
  (mapcar #'(lambda (c)
	      (let ((args (mapcar #'(lambda (arg)
				      (subst-mod-params-constructor-arg
				       arg modinst fbindings))
			    (arguments c))))
...
(defun subst-mod-params-constructor-arg (arg modinst bindings)
  (let* ((dtype (subst-mod-params* (declared-type arg) modinst bindings))
	 (btype (or (cdr (assoc dtype bindings :test #'tc-eq))
		    dtype)))
    (make-instance 'adtdecl
      :id (id arg)
      :module (current-theory)
      :declared-type (parse-unparse btype 'type-expr))))

(defun subst-mod-params-adt-types (type modinst bindings)
  (gensubst type #'(lambda (obj) (subst-mod-params-adt-types! obj modinst bindings))
	    #'(lambda (obj) (subst-mod-params-adt-types? obj modinst bindings))))

(defmethod subst-mod-params-adt-types? ((obj adt-type-name) modinst bindings)
  (assoc obj bindings :test #'tc-eq))

(defmethod subst-mod-params-adt-types? (obj modinst bindings)
  nil)

(defmethod subst-mod-params-adt-types! ((obj adt-type-name) modinst bindings)
  (cdr (assoc obj bindings :test #'tc-eq)))
  ...
  (with-slots (decl-formals formals declared-type type definition def-axiom) ndecl
	...
	(apply-to-bindings #'(lambda (bd) (subst-mod-params* bd modinst bindings))
      ...
      (let ((ntype (substit (subst-mod-params* (type ndecl) modinst bindings) alist))
	    ...
	    (ndef (when (definition ndecl)
		    (substit (subst-mod-params* (definition ndecl) modinst bindings) alist)))
	...
	#+toostrong (assert (or (tc-eq type ntype)
				(with-current-decl ndecl (fully-instantiated? ntype))))
	#+toostrong (assert (or (tc-eq (declared-type ndecl) ndtype)
				(with-current-decl ndecl (fully-instantiated? ndtype))))
	#+toostrong (assert (or (tc-eq (definition ndecl) ndef)
...
(defun subst-mod-params-formal-decls (fdecls decl modinst bindings &optional nfdecls nfacts)
  (if (null fdecls)
      (values (nreverse nfdecls) (nreverse nfacts) bindings)
	...
	(subst-mod-params-formal-decls
	 ...
	 decl
	 modinst
	 (acons (car fdecls) nfact bindings)
	 (cons nfdecl nfdecls)
	 (cons nfact nfacts)))))
...
(defmethod subst-mod-params* ((ndecl formula-decl) modinst bindings)
  (with-slots (definition closed-definition) ndecl
    (assert (generated-by ndecl)) ;; Should be new
    (assert (every #'(lambda (df) (assq df bindings))
		   (decl-formals (generated-by ndecl))))
...
(defmethod subst-mod-params* ((decl auto-rewrite-decl) modinst bindings)
  (let ((rnames (subst-mod-params* (rewrite-names decl) modinst bindings)))
    (setf (rewrite-names decl) rnames)))
...
(defmethod subst-mod-params* ((type adt-type-name) modinst bindings)
  (let ((btype (assoc type bindings :test #'tc-eq)))
    (if btype
	(cdr btype)
	(let* ((ntype (call-next-method))
	       (stype ntype)) ;;(subst-for-formals ntype bindings)
	  (unless (adt stype)
	    (setf (adt stype) *adt*))
	  stype))))
...
(defmethod subst-mod-params* ((type type-name) modinst bindings)
  (let* ((res (car (resolutions type)))
    ...
    #+pvsdebug
    (assert (or (null bdg) (not (fully-instantiated? modinst))
		(fully-instantiated? bdg) (dactuals modinst)))
    #+pvsdebug
    (assert (or (fully-instantiated? bdg) (dactuals modinst)
...
(defmethod subst-mod-params* ((type expr-as-type) modinst bindings)
  (let* (;; (ntype (call-next-method)) ;; subtype will be next
	 (nexpr (subst-mod-params* (expr type) modinst bindings)))
    (if (everywhere-true? nexpr)
	(domain (type nexpr))
	(lcopy type :expr nexpr))))
...
(defmethod subst-mod-params* ((type subtype) modinst bindings)
  (let ((elt (assoc type bindings :test #'formal-subtype-binding-match)))
    (multiple-value-bind (act from-type)
	(or (cdr elt)
	    (and *subst-mod-params-map-bindings*
		 (find-supertype-map-binding type)))
      (if act
	  (cond (elt (type-value act))
		((and *subst-mod-params-map-bindings*
		      (not (compatible? (type-value act) type)))
		 ;;(break "subst-mod-params* subtype not elt")
		 (if (tc-eq (domain (type (predicate type))) (type-value from-type))
		     (let ((spred (subst-mod-params* (predicate type) modinst bindings)))
		       (if (tc-eq spred (predicate type))
			   (type-value act)
			   (lcopy type
			     :supertype (type-value act)
			     :predicate spred
			     :print-type nil)))
		     (type-value act)))
		((strong-tc-eq (type-value act) type)
		 (type-value act))
		((strong-tc-eq (type-value act) type)
		 type)
		(t (lcopy (type-value act) :print-type nil)))
	  (let* ((stype (subst-mod-params* (supertype type) modinst bindings))
		 (ptype (print-type type))
		 (ptdecl (when (and ptype (type-name? ptype)) (declaration ptype)))
		 (spred (when (or t
				  (null (mappings modinst))
				  (compatible? stype (supertype type)))
			  (beta-reduce (subst-mod-params* (predicate type)
							  modinst bindings)))))
	    (when (and *subst-mod-params-map-bindings*
		       ptdecl (type-from-decl? ptdecl)
		       (name-expr? spred)
		       (eq (predicate ptdecl) (declaration spred))
		       (not (compatible? (supertype type) stype)))
	      (type-error (or *resolve-name* type)
		"Uninterpreted subtype ~a should also be mapped" type))
	    (if (or (null spred)
		    (everywhere-true? spred))
		(if (eq (print-type type) (print-type stype))
...
(defmethod find-supertype-map-binding ((type subtype))
  (let ((elt (assoc type *subst-mod-params-map-bindings*
		    :test #'(lambda (x y) (and (type-from-decl? y)
					       (tc-eq x (type-value y)))))))
    (if elt
	(values (cdr elt) (car elt))
	(find-supertype-map-binding (supertype type)))))
    
(defmethod find-supertype-map-binding ((type type-expr))
  nil)

(defmethod formal-subtype-binding-match (subtype (formal formal-subtype-decl))
  (tc-eq subtype (type-value formal)))

(defmethod formal-subtype-binding-match (subtype (formal type-from-decl))
  (tc-eq subtype (type-value formal)))
		     ...
		     (assoc decl bindings
			    :test #'(lambda (d bd)
				      (and (mapping-lhs? bd)
					   (eq d (declaration bd))))))))
    #+pvsdebug
    (assert (or (null dacts) (null (dactuals expr))
		(tc-eq dacts (dactuals expr))))
    #+pvsdebug
    (assert (or (null dacts) (= (length dacts) (length (decl-formals decl)))))
...
(defmethod subst-mod-params* ((bd binding) modinst bindings)
  (let* ((ntype (subst-mod-params* (type bd) modinst bindings))
	 (ndeclared-type (when (and (declared-type bd)
				    (not *generating-mapped-axiom-tcc*))
...
(defmethod subst-mod-params* ((expr projection-expr) modinst bindings)
  (with-slots (actuals dactuals type) expr
    (let ((nacts (when actuals (subst-mod-params* actuals modinst bindings)))
	  (ndacts (when dactuals (subst-mod-params* dactuals modinst bindings)))
	  (ntype (subst-mod-params* type modinst bindings)))
      #+pvsdebug (assert (fully-instantiated? ntype))
      (lcopy expr :actuals nacts :dactuals ndacts :type ntype))))
...
(defmethod subst-mod-params* ((expr projection-application) modinst bindings)
  (with-slots (argument actuals dactuals type) expr
    (let ((nacts (when actuals (subst-mod-params* actuals modinst bindings)))
	  (ndacts (when dactuals (subst-mod-params* dactuals modinst bindings)))
	  (narg (subst-mod-params* argument modinst bindings))
	  (ntype (subst-mod-params* type modinst bindings)))
      #+pvsdebug (assert (fully-instantiated? ntype))
      (lcopy expr :actuals nacts :dactuals ndacts :argument narg :type ntype))))
...
(defmethod subst-mod-params* ((expr rational-expr) modinst bindings)
  (if (integerp (number expr))
      (if (minusp (number expr))
	  (subst-mod-params* (change-class expr 'int-expr) modinst bindings)
	  (subst-mod-params* (change-class expr 'number-expr) modinst bindings))
      (if (or (and (minusp (number expr))
		   (assq (declaration (minus-operator)) bindings))
	      (assq (declaration (divides-operator)) bindings))
	  (let* ((num (numerator (number expr)))
		 (den (denominator (number expr)))
		 (num-ex (if (minusp num)
			     (make!-minus (make!-number-expr (- num)) t)
			     (make!-number-expr num)))
		 (den-ex (make!-number-expr den))
		 (rat-ex (make!-application (divides-operator) num-ex den-ex))
		 (nrat-ex (subst-mod-params* rat-ex modinst bindings)))
	    (if (eq nrat-ex rat-ex)
		expr
		nrat-ex))
	  expr)))

(defmethod subst-mod-params* ((expr int-expr) modinst bindings)
  (if (and (minusp (number expr))
	   (assq (declaration (minus-operator)) bindings))
      (let* ((minus-ex (make!-minus (make!-number-expr (- (number expr))) t))
	     (nex (subst-mod-params* minus-ex modinst bindings)))
	(if (eq minus-ex nex)
	    expr
	    nex))
      expr))
...
(defun number-declaration (number)
  (or (gethash number *number-declarations*)
      (let* ((numtype *number_field*)
	     (decl (make-instance 'const-number-decl
		     :id number
		     :declared-type numtype
		     :type numtype)))
	(assert (type-name? (print-type numtype)))
	(setf (module decl) (module (declaration (print-type numtype))))
...
(defmethod subst-mod-params* ((expr cases-expr) modinst bindings)
  (if (some #'(lambda (sel) (assq (declaration (constructor sel))
      ...
      (let ((nexpr (subst-mod-params* (expression expr) modinst bindings))
	    ...
	    (nelse (when (else-part expr) (subst-mod-params* (else-part expr) modinst bindings)))
	    (type (subst-mod-params* (type expr) modinst bindings)))
	(lcopy expr
	  :expression nexpr
	  :selections nsels
...
(defmethod subst-mod-params* ((expr application) modinst bindings)
  (with-slots (operator argument) expr
    (let* ((eq-ineq? (eq-ineq-expr? operator))
	   (arg (subst-mod-params* argument modinst bindings))
	   (op (unless (and eq-ineq? (not (eq arg argument)))
		 (let ((*smp-include-actuals*
      ...
      (cond ((and eq-ineq?
		  (not (eq arg argument)))
	     #+pvsdebug
	     (assert (compatible? (car (types (type arg))) (cadr (types (type arg)))))
	     (let* ((ctype (find-supertype (car (types (type arg)))))
		    (acts (list (mk-actual ctype)))
		    (res (resolution operator))
		    (mi (module-instance res))
		    (ntype (mk-funtype (mk-tupletype (list ctype ctype)) *boolean*))
		    (nmi (let* ((cmi (copy mi :actuals acts))
				(nmres (copy (resolution mi) :module-instance cmi)))
			   (when nmres
			     (setf (resolutions cmi) (list nmres)))
			   cmi))
		    (nres (copy res :module-instance nmi :type ntype))
		    (nop (copy operator :type ntype :resolutions (list nres))))
	    ...
	    (t (let* ((nop (if (and (implicit-conversion? op)
		      ...
		      (dtype (dep-binding-type (domain (find-supertype optype))))
		      (narg (cond ((compatible? dtype (type arg))
				   arg)
		      ...
		      (nex (if (and (not (let-expr? expr))
				    (lambda-expr? nop))
			       (make!-application nop narg)
			       (lcopy expr :operator nop :argument narg :type rtype))))
...
(defmethod subst-mod-params* :around ((expr table-expr) modinst bindings)
  (let ((nexpr (call-next-method)))
	...
	expr
	(lcopy nexpr
	  :row-expr (when (row-expr nexpr)
		      (subst-mod-params* (row-expr nexpr) modinst bindings))
	  :col-expr (when (col-expr nexpr)
		      (subst-mod-params* (col-expr nexpr) modinst bindings))
	  :row-headings (when (row-headings nexpr)
			  (subst-mod-params* (row-headings nexpr) modinst bindings))
	  :col-headings (when (col-headings nexpr)
			  (subst-mod-params* (col-headings nexpr) modinst bindings))
	  :table-entries (subst-mod-params* (table-entries nexpr) modinst bindings)))))
    ...
    (let ((entry (assoc id bindings
	  ...
	  (cond ((and (eq id (id modinst))
		...
		(t (let* ((nacts (if actuals
			  ...
			  (nmaps (when mappings (subst-mod-params* mappings modinst bindings)))
...
(defmethod subst-mod-params* ((map mapping) modinst bindings)
  (with-slots (lhs rhs declared-type type) map
    ...
    (let ((ntype (subst-mod-params* type modinst bindings))
	  (nres (subst-mod-params* (resolutions lhs) modinst bindings)))
      #+pvsdebug
      (assert (fully-instantiated? nres))
      (lcopy map
	:lhs (lcopy lhs :resolutions nres)
  ...
  (with-slots ((decl declaration) (mi module-instance) type) res
    (let ((acts (actuals mi))
      ...
      (cond ((and (null bdg) (binding? decl))
	     (let* ((ntype (when type (subst-mod-params* type modinst bindings)))
...
(defmethod make-resolution ((decl binding) modinst
  ...
  #+pvsdebug (assert (or (null type)
			 ...
			 (not (fully-instantiated? modinst))
			 (fully-instantiated? type)))
  (let ((mi (if (and modinst (fully-instantiated? modinst))
		modinst
		(current-theory-name))))
    ;;(assert (fully-instantiated? mi))
    (cond (type
	   #+pvsdebug
	   (assert (fully-instantiated? type))
	   (mk-resolution decl mi type))
	  ((eq modinst (current-theory-name))
	   (mk-resolution decl mi (type decl)))
	  (t
	   (let ((stype (subst-mod-params (type decl) mi nil sdecl)))
