...
(defmacro make-c-name (prefix module-id decl-id) ;;NSH(2/5/25)
  `(intern (if ,prefix (pvsid-to-cstring (format nil "~a_~a__~a" ,prefix ,module-id ,decl-id))
	       (pvsid-to-cstring (format nil "~a__~a" ,module-id ,decl-id)))))

(defun pvs2ir-unique-decl-id (decl &optional prefix)
  (let ((module-id (simple-id *theory-id*)) ;(simple-id (id (module decl))))
	(decl-id (simple-id (id decl))))
    ;(when (adt-def-decl? decl) (break "pvs2ir-unique"))
    (if (const-decl? decl)
	(let ((same-id-decls (remove-if
	  ...
	  (if (cdr same-id-decls)
		...
		(make-c-name prefix module-id (format nil "~a_~d"  decl-id idx))) ;;% to avoid name confusion
	    (make-c-name prefix module-id decl-id)))
      (make-c-name prefix module-id decl-id))))
...
(defun mk-ir-let (vartype expr body)
  ;(when (null expr) (break "mk-ir-let null expr"))
...
(defun mk-ir-upgrade (target lhs rhs)
  (make-instance 'ir-upgrade
		 :ir-target target
		 :ir-lhs lhs
		 :ir-rhs rhs))
...
(defun mk-ir-formal-typename (id decl)
...
(defun mk-ir-fieldtype (id type pvs-id);doubles as a variable
...
(defmethod print-ir ((ir-expr ir-upgrade))
  (with-slots (ir-target ir-lhs ir-rhs) ir-expr
    `(upgrade ,(print-ir ir-target) ,(print-ir ir-lhs) ,(print-ir ir-rhs))))
...
(defmethod print-ir ((ir-type ir-arraytype))
  (with-slots (size high ir-range) ir-type
	      (format nil "~a[~d/~a]" (print-ir ir-range) size (format nil "~a" (print-ir high)))))
  ...
  (with-slots (exprs type) expr
    (let* ((type (or expected (type expr)))
	  ...
	  (let* ((elem-type (range (find-supertype type)))
		 ...
		 (ir-exprs (loop for x in exprs collect (pvs2ir* x bindings elem-type)))) 
...
(defun pvs2ir-formal-types (formals ir-actuals bindings)
  (cond ((consp formals)
	 (cond ((formal-const-decl? (car formals))
...
(defmethod pvs2ir* ((expr formal-type-decl) bindings expected)
  ...
  (let ((bnd (assoc expr bindings)))
    (if bnd (if (ir-const-formal? (cdr bnd))
		(mk-ir-formal-typename (ir-name (cdr bnd)) expr)
	      (cdr bnd))
      (pvs2ir-decl* expr))))
...
(defun pvs2ir-type-new-irvars (types bindings)
  (cond ((consp types)
	 (let* ((typ1 (car types))
	   ...
	   (cons ir-var1 (pvs2ir-type-new-irvars (cdr types) new-bindings))))
	(t nil)))
...
(defmethod pvs2ir* ((expr lambda-expr) bindings expected)
  (with-slots ((expr-bindings bindings) expression) expr 
    (let* ((ir-binds (pvs2ir-lambda-bindings expr-bindings bindings))
	   (expected-supertype (when expected (find-supertype expected)))
	   (expected-domain (when expected (domain expected-supertype)))
	   (ir-expected-domvars (when expected
				  (let* ((expected-domtype (if (typep expected-domain 'dep-binding)
							       (type expected-domain)
							       expected-domain))
					 (ir-expected-domtypes (or (and (tupletype? expected-domtype)
								       (> (length ir-binds) 1);if detupled
								       (types expected-domtype))
								  (list expected-domtype))))
				     (pvs2ir-type-new-irvars ir-expected-domtypes bindings))))
	   (ir-var-bindings (pairlis expr-bindings ir-binds))
	   (in-bindings (append ir-var-bindings bindings))
	   (expected-range (or (and expected (dependent-function-range expected-supertype expr-bindings))
...
(defmethod pvs2ir* ((expr application) bindings expected)
...
(defun rm-ir-eval-info (decl)
  (and (eval-info decl)
       (setf (ir (eval-info decl)) nil
	     (cdefn (eval-info decl)) nil
	     (c-type-info-table (eval-info decl)) nil)))


(defun clear-constructor (constr)
  (rm-ir-eval-info (rec-decl constr))
  (loop for accdecl in (acc-decls constr)
	do (rm-ir-eval-info  accdecl))
  (rm-ir-eval-info (con-decl constr)))



(defmethod clear-decl* ((decl type-decl))
  (cond ((adt (type-value decl))
	 (let* ((adt (adt (type-value decl)))
	       (constructors (constructors adt)))
	   (loop for con in constructors
		 do (clear-constructor con)))
	 (setf (ir-type-value decl) nil)
	 decl)
	(t (setf (ir-type-value decl) nil) decl)))
...
(defmethod pvs2ir-decl* ((decl formal-type-decl))
  (let ((ir-type-value (ir-type-value decl)))
	...
	(let* ((ir-type-name-id (pvs2ir-unique-decl-id decl))
	       (ir-type-name (mk-ir-formal-typename ir-type-name-id decl)))
	  (push ir-type-name *ir-type-info-table*)
...
;;generates code for a (partially) monomorphic instance theory of a parent theory
(defun pvs2c-mono-theory-body-step (parent instance)
    (unless (ht-instance-clone parent)
	(setf (ht-instance-clone parent)
...
;;method mk-theory-clone factors out the application of subst-mod-params to create a fresh theory instance
;;and generate code for the instance while updating the instance hashtable of the parent
(defmethod mk-theory-clone ((theory rectype-theory) thinst theory-id)
  (let* ((adt (get-theory (generated-by theory)))
	 (adt-instance (subst-mod-params adt thinst nil nil theory-id))
	 (theory-suffix (subseq (string (id theory)) (1+ (length (string (id adt)))))))
    ;(break "mk-theory-clone")
    (when (adt-theory adt-instance)
      (pvs2c-mono-theory-body-step (adt-theory adt) (adt-theory adt-instance)))
    (when (adt-map-theory adt-instance)
      (pvs2c-mono-theory-body-step (adt-map-theory adt) (adt-map-theory adt-instance)))
    (when (adt-reduce-theory adt-instance)
      (pvs2c-mono-theory-body-step (adt-reduce-theory adt) (adt-reduce-theory adt-instance)))
    (cond ((equal theory-suffix "adt")
	   (adt-theory adt-instance))
	  ((equal theory-suffix "adt_map")
	   (adt-map-theory adt-instance))
	  ((equal theory-suffix "adt_reduce")
	   (adt-reduce-theory adt-instance)))))

(defmethod mk-theory-clone ((theory t) thinst theory-id)
  (let ((instance (subst-mod-params theory thinst nil nil theory-id)))
    (pvs2c-mono-theory-body-step theory instance)
    instance))
...
(defun pvs2ir-constant-ir (expr bindings decl)
  (let* ((theory (module decl))
	 ...
	 (nonref-actuals (loop for actlabel in type-actuals
			       ...
			       collect actlabel))
	 (root-theory-id (let ((genby (generated-by theory)))
				    (or (and genby
					     (module? genby)
					     (id genby))
					(id theory))))
	 (theory-root-suffix (subseq (string (id theory)) (length (string root-theory-id))))
	 (new-theory-id (format nil "~a_~{~a~^_~}" (simple-id root-theory-id)
				type-actuals))
	 (instance-theory-id (intern (format nil "~a~a" new-theory-id theory-root-suffix) :pvs))
    ...
    (cond ((and nonref-actuals (not (eq theory *current-pvs2c-theory*))) ;; was (eq intern-theory-id *theory-id*)
	   (let* ((*theory-id* intern-theory-id)
	     ...
	     (assert (eq (id instdecl)(id decl)));check that instdecl is working correctly
	     (if (and (null (cdefn (eval-info instdecl))) ;;NSH(2/6/25): Added check for call to non-executable operation
		      (not (eq instdecl *pvs2c-current-decl*)))	;guard for recursive decl
		 (pvs2c-warn "Found non-executable operation ~a while code generating for ~a" (id instdecl)(id *pvs2c-current-decl*))
		 (ir (eval-info instdecl)))))
	  (t (if (and (null (cdefn (eval-info decl))) ;;NSH(2/6/25): Added check for call to non-executable operation
...
(defun pvs2ir-adt (adt)
  (let* ((adt-decl (declaration adt)))
    (unless (and (ir-type-value adt-decl)(ir-type-name (ir-type-value adt-decl)))
	(pvs2ir-adt-decl adt-decl))))

(defun pvs2ir-adt-decl (adt-decl);;only called with ir-type-value empty
  (let* ((adt-type (type-value adt-decl))
	 (adt-type-id (pvs2ir-unique-decl-id adt-decl))
	 (adt-adt-id (format nil "~a_adt" adt-type-id))
	 (adt (adt adt-type))
	 ;(formals (formals-sans-usings (module adt-decl)))
	 (constructors (constructors adt))
	 (index-id (intern (format nil "~a_index" adt-adt-id)))
	 (index-type (mk-ir-subrange 0 (1- (length constructors))))
	 (adt-type-name (mk-ir-typename adt-adt-id nil ;dummy placeholder to block recursion
					     *ir-theory-formals* nil adt-decl)))
    (setf (ir-type-value adt-decl);;the ir-type-def is set below
	  (mk-eval-type-info adt-type-name))
    (let ((adt-enum-or-record-type
	    (if (enumtype? adt)
		index-type
		(mk-ir-adt-recordtype (list (mk-ir-fieldtype index-id
							     index-type;NSH(5/18/26): _index as PVS-ID
							     '|_index|))
				      nil))));;set constructors following pvs2ir-adt-constructor
      (setf (ir-type-defn (ir-type-name (ir-type-value adt-decl)))
...
(defun pvs2ir-adt-enum-constructor
  ...
  (let* ((cdecl (con-decl constructor))
	 ...
	 (cid (pvs2ir-unique-decl-id cdecl "enum"))
...
(defun pvs2ir-adt-record-constructor (constructor index index-id index-type adt-recordtype adt-type-name)
  (let* ((cdecl (con-decl constructor))
	 ...
	 (constructor-ir-id (intern (format nil "con_~a" cid)))
	 (einfo (get-eval-info cdecl))
	 (*var-counter* nil))
    (newcounter *var-counter*)
    (setf (ir einfo) (make-instance 'ir-constructor-defn)
      ...
      (let* ((indexvar (mk-ir-variable (new-irvar) index-type))
	     ...
	     (full-cargs (append *ir-theory-formals* cargs));;NSH(4/13/25)
	     ...
	     (ctypename (if args (mk-ir-typename cid  cbody-recordtype *ir-theory-formals* nil cdecl) adt-type-name));;need to add params
...
(defun pvs2ir-adt-enum-recognizer (rdecl index index-id index-type 
  ...
  (let* ((einfo (get-eval-info rdecl))
    ...
    (newcounter *var-counter*)
    (or ir-einfo
	(let* ((rid (pvs2ir-unique-decl-id rdecl "rec"))
	       ...
	       (rbody (mk-ir-let index-var (mk-ir-integer index)
				 check-expr))
	       (recognizer-name (mk-ir-function  rid rdecl)))
...
;    (or ir-einfo)
    (let* ((rid (pvs2ir-unique-decl-id rdecl "rec"))
	   ...
	   (full-rargs (append *ir-theory-formals* rargs));;NSH(4/13/25)
      ...
      (setf (ir einfo)(mk-ir-defn recognizer-name full-rargs '|bool| rbody)
	    (cdefn einfo) nil
	    (c-type-info-table einfo) nil)
      (ir einfo))))
...
;	       (adefn (mk-ir-lambda (list aargvar) accessor-ir-type abody))
	       ...
	       (accessor-name (mk-ir-function (format nil "acc_~a_~a" (ir-type-id adt-type-name) adecl-id) adecl))
	  ...
	  (setf (ir einfo)    ;;NSH(4/13/25)
		(mk-ir-accessor-defn accessor-name (append *ir-theory-formals* (list aargvar)) accessor-ir-type abody
				     (mk-ir-defn update-name (append *ir-theory-formals* (list aargvar new-value-var)) ctype ubody))
    ...
    (cond ((consp  acc-constructor-index-decls)
	   (let* ((cindex (caar acc-constructor-index-decls))
		  ...
		  (cbranch (let ((cast-var (mk-ir-variable (new-irvar)
			     ...
			     (mk-ir-let cast-var  aargvar (mk-ir-get  cast-var adecl-id)))))
	   ...
	   (let* ((cindex (caar acc-constructor-index-decls))
		  ...
		  (cbranch (let ((cast-var (mk-ir-variable (new-irvar)
			     ...
			     (mk-ir-let cast-var  aargvar
					(mk-ir-constructor-update cast-var adecl-id new-value-var)))))
	     (if (consp (cdr acc-constructor-index-decls))
		 (let ((condvar (mk-ir-variable (new-irvar) '|bool|))
		       (indvar (mk-ir-variable (new-irvar) '|uint32|))
		       (intvar (mk-ir-variable (new-irvar) '|uint32|)))
	 ...
	 (let* ((var1 (car let-bindings))
		...
		(ir-body (pvs2ir-let-expr (cdr let-bindings) (cdr args)
				       ...
				       (acons var1
					      ...
					      bindings)
				       expected)))
	   ;(when (ir-funtype? ir-type) (break "pvs2ir-let-expr(funtype)"))
...
(defmethod pvs2ir* ((expr record-expr) bindings expected)
...
(defmethod pvs2ir-expr-type ((expr tuple-expr) &optional bindings)
  (with-slots (exprs) expr
	      (mk-ir-tupletype (loop for ir-type in (pvs2ir-expr-type exprs bindings)
				      as i from 1
				      collect (let ((proj-fieldname (intern (format nil "project_~a" i))))
						(mk-ir-fieldtype proj-fieldname
							       ir-type
							       proj-fieldname))))))
...
;; (defmethod pvs2ir-expr-type ((expr array-expr) &optional bindings)
;;   (with-slots (type) expr
;;     (let*
;;     (mk-ir-arraytype 
...
(defun pvs2ir-field-assignments (assignments bindings expected 
  ...
  (cond ((consp assignments)
	 (let* ((assgn1 (car assignments))
		...
		(ir-record-fieldtype (mk-ir-fieldtype (ir-fieldname ir-field) ir-field-type (ir-fieldname ir-field)))
	...
	(t ;; (let* ((print-expected-record-type (and expected-record-type (print-type expected-record-type)))
	   ...
	   ;; 						       ir-actuals)))))
	 (mk-ir-let* (nreverse accum-ir-fieldvars)  ;expected type is used (nreverse accum-ir-expr-types)
...
(defmethod pvs2ir* ((expr field-application) bindings expected)
  ...
  (with-slots (id argument) expr
	      (let* ((ir-argument (pvs2ir* argument bindings nil));don't know the exact record type of argument
		     ...
		     (argirtype (pvs2ir-type argtype bindings))
		     (exprirtype (pvs2ir-type (type expr) bindings))
...
(defun pvs2ir-update (assignments ir-expression expression-type bindings)
  (let ((rhs-list (mapcar #'expression assignments))
    ...
    (let* ((rhs-irvar-list (loop for i from 1 to (length rhs-list)
	   ...
	   (ir-exprvar (mk-ir-variable (new-irvar) expression-type))
	   (*update-lhs-vars* nil)
	   (*update-lhs-bindings* nil)) ;binds the ir-expression value
      (let ((ir-update-expr (pvs2ir-assignments lhs-list ;build updates
						...
						bindings
						maplet-list)))
	(let ((lhs-vars (nreverse *update-lhs-vars*))
	      (lhs-bindings (nreverse *update-lhs-bindings*)))
	  (make-ir-let* lhs-vars lhs-bindings
...
(defun pvs2ir-assignments (lhs-list rhs-irvar-list
			       ir-exprvar ir-expr-type bindings maplet-list)
  (cond ((consp lhs-list)
	 (let ((ir-assignment1
...
;;    en = e11 WITH [j := v]
;; in e1 WITH [i := en]
(defmethod pvs2ir-assignment1 ((ir-expr-type ir-funtype) lhs rhs-irvar ir-exprvar bindings maplet?)
  (cond ((consp lhs);;NSH(2-14-25): added maplet? but this is only relevant for arrays.
	 (let* ((ir-exprvar11 (new-irvar)) 
	   ...
	   (let ((lhs1-irvar (new-irvar))
	     ...
	     (let ((ir-lhs1-vartype (mk-ir-variable lhs1-irvar lhs1-ir-type))
	       ...
	       (push ir-lhs1-vartype *update-lhs-vars*);pushing lhs var/binding to get the order of evaluation right
	       (push lhs1-ir *update-lhs-bindings*)
...
(defmethod pvs2ir-assignment1 ((ir-expr-type ir-arraytype) lhs rhs-irvar ir-exprvar bindings maplet?)
  (cond ((consp lhs)
	 (let* ((ir-exprvar11 (new-irvar))
		(ir-expr-type11 (ir-range ir-expr-type))
		(ir-expr-vartype11 (mk-ir-variable ir-exprvar11 ir-expr-type11))
			  ...
			  (mk-ir-let ir-expr-vartype1
					 ...
					 (let* ((ir-nullvar (new-irvar))
					   ...
					   (mk-ir-let ir-nullvartype
						      (mk-ir-nil)
						      (if maplet?
							  (mk-ir-upgrade ir-exprvar ir-lhs1-vartype ir-nullvartype)
							  (mk-ir-update ir-exprvar ir-lhs1-vartype ir-nullvartype))))
				       ir-exprvar)
				     (mk-ir-let ir-new-rhsvartype
						ir-rest-assignment
						(if maplet?
						    (mk-ir-upgrade ir-expr-vartype1 ir-lhs1-vartype ir-new-rhsvartype)
						  (mk-ir-update ir-expr-vartype1 ir-lhs1-vartype ir-new-rhsvartype)))))))))
	(t ir-exprvar)))

(defmethod pvs2ir-assignment1 ((ir-expr-type ir-recordtype) lhs rhs-irvar ir-exprvar bindings maplet?)
    (cond ((consp lhs)
...
(defmethod pvs2ir-assignment1 ((ir-expr-type ir-adt-recordtype) lhs rhs-irvar ir-exprvar bindings maplet?)
    (cond ((consp lhs)
	   (let* ((lhs1 (caar lhs));;lhs1 is a field-name-expr
		  ;(ir-field-decl (find (id lhs1) (ir-field-types ir-expr-type) :key #'ir-id))
...
(defmethod pvs2ir-assignment1 ((ir-expr-type ir-typename) lhs rhs-irvar ir-exprvar bindings maplet?)
  (pvs2ir-assignment1 (ir-type-defn ir-expr-type) lhs rhs-irvar ir-exprvar bindings maplet?))
    ...
    (cond ;((tc-eq type *string-type*) (mk-ir-stringtype))
	  ...
	  ((type-name? type-name)
	   (let* ((type-decl (declaration type-name))
		  (theory (module type-decl))
		  (thinst (module-instance type-name))
		  (actuals (actuals thinst))
		  (type-actuals (loop for act in actuals
				      when (type-value act)
				      collect (check-actual-type (type-value act) tbinding)))
		  (nonref-actuals (loop for actlabel in type-actuals
					when (not (eq actlabel 'ref))
					collect actlabel))
		  (root-theory-id (let ((genby (generated-by theory)))
		  ...
		  (if nonref-actuals
		      (let* ((*theory-id* intern-theory-id)
			     ...
			     (theory-instance
				  ...
				  (mk-theory-clone theory thinst intern-theory-id)))
			     (dummy2 (when thclone (format t "~%Found thclone")))
			     (inst-decl (loop for dc1 in (theory theory)
					     as dc2 in (theory theory-instance)
					     when (eq dc1 type-decl)
					     return dc2))
			     )
			(if (ir-type-value inst-decl)
			    (ir-type-name (ir-type-value inst-decl))
			    (call-next-method)))
		      (call-next-method)))))
	   (t (call-next-method)))))
    ...
    (let* ((ir-dom (pvs2ir-type* domain tbinding))
	   ...
	   (new-tbinding (if (binding? domain)
			    (acons domain (mk-ir-variable new-irvar
				   ...
				   tbinding)
			   tbinding))
	   (new-ir-dom (if (binding? domain) (mk-ir-fieldtype new-irvar ir-dom '|_domain|) ir-dom)))
...
(defmethod pvs2ir-type* ((type field-decl) tbinding)
  (mk-ir-fieldtype (id type)(pvs2ir-type* (type type) tbinding)(id type)))
...
(defun pvs2ir-tuplefields (types tbinding &optional  tuple-label (index 1))
  (cond ((consp types)
	 (let* ((field-id (intern (format nil "project_~a" index)))
		(car-irtype (pvs2ir-type* (car types) tbinding))
		(car-ir-fieldtype (mk-ir-fieldtype field-id car-irtype field-id))
...
(defun ir2c-tcompatible-but (texpr1 texpr2)
  ;;same as ir2c-tcompatible but checks that both definitions are constructor-record types.
...
(defmethod preprocess-ir* ((ir-expr ir-upgrade) livevars bindings)
  (with-slots (ir-target ir-lhs ir-rhs) ir-expr
	      (let ((new-ir-target (preprocess-ir* ir-target livevars bindings))
		    (new-ir-lhs (preprocess-ir* ir-lhs livevars bindings))
		    (new-ir-rhs (preprocess-ir* ir-rhs livevars bindings))) 
		(unless (or *to-emacs*
			    (ir-last? new-ir-target))
		  (format t "~%Updating an unmarked variable in ~s" (print-ir ir-expr)))
	      (mk-ir-upgrade new-ir-target new-ir-lhs new-ir-rhs))))
...
(defun ir2c-function-apply (return-var return-type ir-function ir-args)
    (let* ((ir-arg-vars (loop for ir-var in ir-args
	  ...
	  (let ((instrs (ir2c-primitive-apply return-var return-type ir-function-name ir-arg-vars ir-arg-var-names))
		;;The primitive GMP operations don't clear input bignums, but perhaps they should.
		;;It's easier to collect the clear instructions here in one place they spread them through the primitives.
  ...
  (with-slots (ir-vartype ir-bind-type ir-bind-expr ir-body) ir-expr
	      (with-slots (ir-name ir-vtype ir-pvsid) ir-vartype
			  (let* ((ir2c-vtype (ir2c-type ir-vtype))
				 ...
				 (bind-instrs 
				   (copy-type ir2c-vtype ir2c-btype ir-name
...
(defmethod ir2c-type ((ir-type ir-const-formal)) 
...
(defmethod ir2c* ((ir-expr ir-update) return-var return-type)
  (with-slots (ir-target ir-lhs ir-rhs) ir-expr
	      (let* ((target-var (get-ir-last-var ir-target))
		     ...
		     (theory-params *ir-theory-formals*)
		     (c-param-arg-string(if (ir-reference-type? rhs-vtype);no params for non-ref update values
					    (format nil "~{, ~a~}" (loop for ir-formal in
									 theory-params
									 collect (ir-formal-id ir-formal)))
					  (format nil "")))
		     (rhs-last (and (not (ir-constructor-update? ir-expr))
				    (ir-last? ir-rhs)
				    (ir-reference-type? rhs-vtype)))
		...
		(if (ir-lvariable? ir-lhs)  ;;this doesn't work with names
		    ...
		    (let* ((lhs-var (get-ir-last-var ir-lhs))
		      ...
		      (if target-last
			  (cons (format nil "~a = (~a_t)~a_~a(~a, ~a, ~a~a)"
					return-var creturn-type
					(if (ir-upgrade? ir-expr) "upgrade" "update")
					ctype target-var-name lhs-var-name
					rhs-var-name c-param-arg-string)
				rhs-last-instr)
		  ...
		  (if target-last
		      (cons (format nil "~a = (~a_t)update_~a_~a(~a, ~a~a)"
			    ...
			    rhs-last-instr)
		    (cons (format nil "{~a = (~a_t)copy_~a(~a); ~a = (~a_t)update_~a_~a(~a, ~a~a);}"
				  return-var creturn-type ctype target-var-name
				  return-var creturn-type ctype ir-lhs return-var rhs-var-name c-param-arg-string)
			  rhs-last-instr))))))
...
(defun make-nested-assignment (lhs lhs-type rhs rhs-type)
  (let ((mpnumber-type (mpnumber-type? lhs-type)))
    (if mpnumber-type
	(list (format nil "~a_init(~a); ~a;" lhs-type lhs
		(make-c-assignment lhs  lhs-type rhs rhs-type)))
	(make-c-assignment-with-count lhs  lhs-type rhs rhs-type))))
...
(defun make-closure-copy-info (type-name-root c-funtype
			       ir-domaintype c-domaintype
			       ir-rangetype c-rangetype ir-freevars)
...
(defun make-closure-release-info (funtype-root closure-name-root
				  ir-domaintype c-domaintype
				  ir-rangetype c-rangetype
				  ir-fvar-types ir-fvar-ctypes
					       c-param-arg-string
  ...
  (let* ((release-name (intern (format nil "release_~a" closure-name-root)))
	 ...
	 (release-defn (let* ((release-fields (loop for ft in ir-fvar-types
						   ...
						   collect (make-release-call ft cft
									      ...
									      c-param-arg-string)))
			      (release-key (make-release-call ir-domaintype c-domaintype
							      "x->htbl->data[j].key" c-param-arg-string))
			      (release-elem (make-release-call ir-rangetype c-rangetype
							      "x->htbl->data[j].value" c-param-arg-string))
			      (release-htbl (if (equal release-key "")
						(if (equal release-elem "")
						    (format nil "")
						    (format nil "for (uint32_t j = 0; j < x->htbl->size; j++){~a;}"
							     release-elem))
						(if (equal release-elem "")
						    (format nil "for (uint32_t j = 0; j < x->htbl->size; j++){~a;}"
							     release-key)
						    (format nil "for (uint32_t j = 0; j < x->htbl->size; j++){~a; ~a;}"
							     release-key release-elem)))))

			 ;;(4/10/25): was  (format nil "release_~a(x->fvar_~a~a)" cft i c-param-arg-string)
			 ;;(8/7/21)This assumes that all reference types are locally (re-)defined.
			 ;;Need to revisit this if types are parametric.  
			 (format nil "void release_~a(~a_t closure~a){~%~8T~a_t x = (~a_t)closure;~
~%~8Tx->count--;~%~8Tif (x->count <= 0){~{~%~8T~8T~a;~}~
~%~10Tif (x->htbl != NULL){~
~%~15T~a
~%~15Tsafe_free(x->htbl->data);
~%~15Tsafe_free(x->htbl);
~%~15T}
~%~8T//printf(\"\\nFreeing\\n\");~%~8Tsafe_free(x);}}"
				 closure-name-root funtype-root c-param-decl-string closure-name-root closure-name-root
				 release-fields release-htbl)))) ;;reminder: release htbl; done
...
(defmacro push-new-type-info (c-type-info info-table decl-table)
  `(let* ((c-type-info ,c-type-info)
	  (ir-texpr (ir-texpr c-type-info))
	  (known-c-type-info (get-c-type-info ir-texpr)))
  ...
  (when (null (ir-type-value decl))
    (let ((ir-type-name (mk-ir-typename (pvs2ir-unique-decl-id decl) nil nil nil decl)));;added params
      ...
      (setf (ir-type-value decl)
	    (mk-eval-type-info ir-type-name))))
  (let ((ir-type-value (ir-type-value decl)))
    (push-new-type-info c-type-info *c-type-info-table* (c-type-info-table ir-type-value))))
  ...
  (with-slots (ir-domain ir-range) ir2c-type
		...
		(or c-type-name
			...
			(let* ((hash-entry-type-defn
				...
				(dupdate-info (when hashtype (make-function-dupdate-info type-name-root ir-domain c-domain-root hashtype c-range-root elemtype c-param-arg-string c-param-decl-string)))
...
~8T}"
			 type-name-root
			 (let ((*c-scope-string* "~8T"))(print2c keyzero-instrs))
			 (let ((*c-scope-string* "~8T"))(print2c keymatch-instrs))
			 (let ((*c-scope-string* "~16T"))(print2c keyzero-instrs))
			 (let ((*c-scope-string* "~16T"))(print2c keymatch-instrs))
			 ))
	 (defn (format nil "~a~a" header body)))
...
(defun make-function-dupdate-info (type-name-root ir-domain c-domain-root htype c-range-root elemtype c-param-arg-string c-param-decl-string)
  (let* ((name (format nil "dupdate_~a" type-name-root))
	 (header (format nil "~a_t ~a(~a_t a, ~a_t i, ~a_t v~a)" type-name-root name type-name-root (mppointer-type c-domain-root) (mppointer-type c-range-root) c-param-decl-string))
	 (body (function-destructive-update-body type-name-root ir-domain  c-domain-root htype c-range-root elemtype c-param-arg-string))
	 (defn (format nil "~a~a" header body)))
    (mk-c-defn-info name  (format nil "~a;" header) defn (list type-name-root c-domain-root c-range-root) type-name-root)))
...
(defun function-destructive-update-body (type-name-root ir-domain c-domain-root htype c-range-root elemtype c-param-arg-string)
...
~%}"
	  ...
	  (ir2c-initialize "new_data[j].key" c-domain-root 0 '|uint8|);line 18 initializes new_data[j].key
	  (let ((*c-scope-string* "~24T")) ;line 23 initialize keyzero
	    (print2c (ir2c-arith-relations '== "keyzero" (list "data[j].key"  0)
				(list c-domain-root '|uint8|))))
	  (let ((*c-scope-string* "~32T"));line 27 initializes newkeyzero
	    (print2c (ir2c-arith-relations '== "newkeyzero" (list "new_data[new_loc].key"  0)
				(list c-domain-root '|uint8|))))
	  (let ((*c-scope-string* "~40T"));line 31 resets newkeyzero in the while loop
	    (print2c (ir2c-arith-relations '== "newkeyzero" (list "new_data[new_loc].key"  0)
				(list c-domain-root '|uint8|))))
	  (make-c-assignment "new_data[new_loc].key" c-domain-root "data[j].key" c-domain-root);line 33
	  (make-c-assignment "new_data[new_loc].value" c-range-root  "data[j].value" c-range-root);line 35
	  (make-function-key-value-release-code "data[j]" ir-domain c-domain-root elemtype c-range-root c-param-arg-string)
	  htype ;line 41: compute hash
	  type-name-root;line 42: look up hashentry
	  type-name-root;line 43: retrieve hashentry
	  (let ((*c-scope-string* "~8T"));line 46: initialize hentrykeyzero
	    (print2c (ir2c-arith-relations '== "hentrykeyzero" (list "hentry.key"  0)
				(list c-domain-root '|uint8|))))
	  (make-c-assignment "htbl->data[hashindex].key" c-domain-root "i" c-domain-root);line 48
	  (make-c-assignment-with-count "htbl->data[hashindex].value" c-range-root "v" c-range-root);line 48 list
	  (if (ir-reference-type? elemtype) ;line 49
...
(defun make-json-call (ir-type c-ir-type expr c-param-arg-string)
  (if (ir-formal-typename? ir-type)
	...
	(if ir-var-for-type-binding
	    (format nil "~a->json_ptr(~a, ~a)" c-ir-type expr (ir-name (cdr ir-var-for-type-binding)))
	    (break "Missing theory binding for formal: ~a" c-ir-type)))
      (if (ir-typename? ir-type)
	  (if (ir-formals ir-type)
	      (if (ir-actuals ir-type);;else case must be a local type, hence add params
		  (break "Not implemented.")
		  (format nil "json_~a((~a_t)~a~a)"  c-ir-type c-ir-type expr c-param-arg-string))
	      ;;if no formals, the add params for local types and omit them for non-local ones
	      (if (eq (module (type-declaration ir-type)) *current-pvs2c-theory*)
		  (format nil "json_~a((~a_t)~a~a)"  c-ir-type c-ir-type expr c-param-arg-string)
		  (format nil "json_~a((~a_t)~a)"  c-ir-type c-ir-type expr)))
	  (if (ir-reference-type? ir-type);;must be a local type
	      (format nil "json_~a((~a_t)~a~a)"  c-ir-type c-ir-type expr c-param-arg-string)
	      (format nil "json_~a(~a)" c-ir-type expr)
	      ))))

(defun make-equal-call (ir-type c-ir-type clhs crhs c-param-arg-string)
  (if (ir-formal-typename? ir-type)
      (let ((ir-var-for-type-binding (assoc (type-declaration ir-type) *ir-theory-tbindings*)))
	(if ir-var-for-type-binding
	    (format nil "~a->equal_ptr(~a, ~a, ~a)" c-ir-type clhs crhs (ir-name (cdr ir-var-for-type-binding)))
	    (break "Missing theory binding for formal: ~a" c-ir-type)))
      (if (ir-typename? ir-type)
	  (if (ir-formals ir-type)
	      (if (ir-actuals ir-type)
		  (break "Not implemented.")
		  (format nil "equal_~a((~a_t)~a, (~a_t)~a~a)"  c-ir-type c-ir-type clhs c-ir-type crhs
			  c-param-arg-string))
	      (if (eq (module (type-declaration ir-type)) *current-pvs2c-theory*)
		  (format nil "equal_~a((~a_t)~a, (~a_t)~a~a)"  c-ir-type c-ir-type clhs c-ir-type crhs
			  c-param-arg-string)
		  (format nil "equal_~a((~a_t)~a, (~a_t)~a)"  c-ir-type c-ir-type clhs c-ir-type crhs)))
	  (if (ir-reference-type? ir-type)
	      (format nil "equal_~a((~a_t)~a, (~a_t)~a~a)"  c-ir-type c-ir-type clhs c-ir-type crhs c-param-arg-string)
	      (if (mpnumber-type? ir-type)
		  (format nil "(~a_cmp(~a, ~a) == 0)" c-ir-type clhs crhs)
		  (format nil "~a == ~a" clhs crhs))
	      ))))
...
(defun make-release-call (ir-type c-ir-type c-expr c-param-arg-string)
  (if (ir-formal-typename? ir-type)
      (let ((ir-var-for-type-binding (assoc (type-declaration ir-type) *ir-theory-tbindings*)))
	(if ir-var-for-type-binding
	    (format nil "~a->release_ptr(~a, ~a)" c-ir-type c-expr  (ir-name (cdr ir-var-for-type-binding)))
	    (break "Missing theory binding for formal: ~a" c-ir-type)))
    (if (ir-typename? ir-type)
	(if (ir-formals ir-type)
	    (if (ir-actuals ir-type)
		...
		(let ((ir-type-defn (ir-type-defn ir-type)))
		  (make-release-call ir-type-defn 
				   (add-c-type-definition (ir2c-type ir-type-defn)) c-expr c-param-arg-string))))
	(if (ir-reference-type? ir-type)
	    (format nil "release_~a((~a_t)~a~a)"  c-ir-type c-ir-type c-expr c-param-arg-string)
...
(defun make-function-key-value-release-code (entry-string ir-domain c-domain-root elemtype c-range-root c-param-arg-string)
  (let ((key-release (make-release-call ir-domain c-domain-root (format nil "~a.key" entry-string) c-param-arg-string))
	(value-release (make-release-call elemtype c-range-root (format nil "~a.value" entry-string) c-param-arg-string)))
    (if (equal key-release "")
	(if (equal value-release "")
	    ""
	    (format nil "for (uint32_t j=0; j < size; j++){~a;}" value-release))
	(if (equal value-release "")
	    (format nil "for (uint32_t j=0; j < size; j++){~a;}" key-release)
	    (format nil "for (uint32_t j=0; j < size; j++){~a; ~a;}" key-release value-release)))))
    ...
    (let* ((c-type-info (get-c-type-info ir2c-type))
	  ...
	  (let* ((c-field-types (loop for ft in ir-field-types
		 ...
		 (type-defn (format nil "struct ~a_s {~%~8Tuint32_t count; ~{~%~8T~a;~}};~%typedef struct ~a_s * ~a_t;"
			      type-name-root c-field-decls type-name-root type-name-root))) 
	    (let* ((theory-params *ir-theory-formals*)
		   (theory-formals *theory-formals*)
		   (theory-c-params (ir2c-theory-formals theory-params theory-formals))
		   (theory-c-params-variadic (ir2c-theory-formals-variadic "actual" theory-params theory-formals))
...
(defun make-adt-record-equal-defn (type-name-root ir-field-types c-field-types constructors
  ...
  (let ((equal-constructor-instrs (loop for constructor in constructors
					...
					when (cdr constructor)
					collect
					(format nil "case ~a: tmp = tmp && ~a; break"
						index (make-equal-call (car constructor)
								       (ir-type-id (car constructor))
								       "x" "y"
								       c-param-arg-string)))))
...
(defun make-adt-record-json-defn (type-name-root ir-field-types c-field-types constructors
  ...
  (let ((json-constructor-instrs (loop for constructor in constructors
					...
					when (cdr constructor)
					collect
					(format nil "case ~a: tmp = safe_strcat(tmp, ~a); break"
						index
						(make-json-call
						 (car constructor)
						 (ir-type-id (car constructor))
...
(defun make-record-equal-defn (type-name-root ir-field-types c-field-types c-param-arg-string c-param-decl-string)
  (let ((equal-field-instrs
	 (loop for ft in ir-field-types
	       as cft in c-field-types
	       collect
	       (format nil "tmp = tmp && ~a" (make-equal-call (ir-vtype ft) cft
							      (format nil "x->~a" (ir-id ft))
							      (format nil "y->~a" (ir-id ft))
							      c-param-arg-string)))))
    (format nil "bool_t equal_~a(~a_t x, ~a_t y~a){~%~8Tbool_t tmp = true;~{~8T~a;~}~%~8Treturn tmp;}"
	...
	(json-field-instrs
	 (loop for ft in ir-field-types
	       ...
	       collect (let* ((stringft (string (ir-id ft)))
			 ...
			 (format nil "~a;~%~8Ttmp[~a] = safe_strcat(~a, ~a)"
				     stmt
				     index
				     fldvar
				     (make-json-call (ir-vtype ft) cft
						     (format nil "x->~a" stringft)
						     c-param-arg-string))))))
...
;; (defun ir2c-tcompatible-expr (texpr ir-expr)
;;   (ir2c-tcompatible-expr* texpr ir-expr))

;; (defmethod ir2c-tcompatible-expr* ((texpr t)(ir-expr ir-bool))
;;   (eq texpr '|bool|))

;; (defmethod ir2c-tcompatible-expr* ((texpr t)(ir-expr ir-string))
;;   (eq texpr '|string|))

;; (defmethod ir2c-tcompatible-expr* ((texpr t)(ir-expr ir-char))
;;   (eq texpr '|char|))

;; (defmethod ir2c-tcompatible-expr* ((texpr t)(ir-expr ir-variable))
;;   (with-slots (ir-vtype) ir-variable
;;     (ir2c-tcompatible texpr ir-vtype)))

;; (defmethod ir2c-tcompatible-expr* ((texpr t)(ir-expr ir-apply))
;;   (ir2c-tcompatible texpr (ir-atype ir-expr)))

;; (defmethod ir2c-tcompatible-expr* ((texpr t)(ir-expr ir-let))
;;   (with-slots (ir-body) ir-expr
;;     (ir2c-tcompatible-expr* texpr ir-body)))

;; (defmethod ir2c-tcompatible-expr* ((texpr t)(ir-expr ir-record))
;;   (with-slots (ir-recordtype) ir-expr
;;     (ir2c-tcompatible texpr ir-recordtype)))

;; (defmethod ir2c-tcompatible-expr* ((texpr ir-funtype)(ir-expr ir-lambda))
;;   (with-slots (ir-vartypes ir-rangetype) ir-expr
;;     (with-slots (ir-domain ir-range) texpr
;;       (and (ir2c-tcompatible ir-domain (if (singleton? ir-vartypes) (ir-vtype (car ir-vartypes))
;; 						 (mapcar #ir-vtype ir-vartypes)))
;; 	   (ir2c-tcompatible ir-range ir-rangetype)))))
...
(defmethod copy-type* ((texpr1 ir-arraytype)(texpr2 ir-funtype) lhs rhs)
  (with-slots ((size1 size)(high1 high)(et1 ir-range)) texpr1
    (with-slots ((dom2 ir-domain)(et2 ir-range)) texpr2 
      (let* ((index (new-indvar))
	     (tmp (gentemp "tmp"))
	     (size-instrs (cons (format nil "uint32_t ~a" tmp)
				(if (ir-offset? high1)
				    (with-slots (expr offset) high1 
	 ...
	 (cond ((if-instr? (car c-instrs))
		(with-slots (if-cond then-instr else-instr) (car c-instrs)
			    (let* ((then-part
				   ...
				   (if-string (format nil "~aif (~a){~a~%~a} else {~a}"
						      *c-scope-string*
						      if-cond
						      then-part
						      *c-scope-string*
