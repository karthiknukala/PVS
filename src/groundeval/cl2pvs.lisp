...
(defun dom-subrange? (sexpr type)
  (if #+allegro (excl::simple-array-p sexpr)
      ...
      (let* ((typeof (type-of sexpr));;returns (VECTOR type len)
...
;; xt-string-to-codes and next-string-code have been restored from parser.lisp
;; (commit 255dcf28ea60be046fd869ccf2e45c2475c96383) and the first one has
;; been renamed as cl-string-to-codes

(defun cl-string-to-codes (string pos len place codes)
  (if (>= pos len)
      (nreverse codes)
	...
	(let* ((strow (starting-row place))
	  ...
	  (cl-string-to-codes string npos len place ncodes)))))
...
(defun cl2pvs*-string (str)
  (let ((ne (mk-name-expr '|char?|))
	(charcodelist (if (stringp str)   ;;NSH(12/21/19): extracts list of character codes
			  (cl-string-to-codes str 0 (length str) '#(0 0 0 0) nil)
