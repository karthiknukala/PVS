...
(defun make-tcc-comment (kind expr type decl ndecl)
  (let* ((submsg (case kind
		   (actuals nil)
		   (assuming (format nil "generated from assumption ~a.~a"
			       (id (module type)) (id type)))
		   ...
		   (termination nil)
		   (well-founded (format nil "for ~a" (id decl)))))
	 (preason (or (cdr (assq expr *compatible-pred-reason*))
		      (when (rec-application-judgement? (current-declaration))
			"Recursive judgement")))
	 (place (or (place *set-type-actuals-name*)
		    (place* expr) (place type)
	 ...
	 (too-long (or (> (length place) 4)
		       (> (+ (length preason) (length (string kind)) (length plstr)
			     (length termstr) 25)
			  *default-char-width*))))
    (format nil "~@(~@[~a ~]~a~) TCC generated ~@[~a~]for~
                 ~:[ ~;~%    %~]~a~
                 ~@[~%    %   (generated by ~a)~]~
                 ~@[~%    % ~a~]"
      preason kind plstr too-long termstr gen-by submsg)))
...
(defmethod nonempty-formula-type ((ex conjunction))
  (let ((car-type (nonempty-formula-type (args1 ex))))
    (when car-type
      (let ((cdr-type (nonempty-formula-type (args2 ex))))
	(when cdr-type
	  (mk-tupletype (list car-type cdr-type)))))))
