...
(defparameter *yices-quant-expand-limit* 256)
...
(defmethod translate-to-yices2* ((expr binding-expr) bindings)
  (translate-to-yices-binding-expr expr bindings))

(defun translate-to-yices-binding-expr (expr bindings)
...
(defun translate-to-yices2-forall (expr-bindings expression bindings)
  (cond (expr-bindings
	 (let* ((fst (car expr-bindings))
		(fsttype (type fst))
		(below (simple-below? fsttype))
		(belowval (when below (pvseval-integer below)))
		(upto (simple-upto? fsttype))
		(uptoval (when upto (pvseval-integer upto)))
		(bnd (or (and belowval (1- belowval)) uptoval))
		(rst (cdr expr-bindings)))
	   (cond (bnd
		  (and (< bnd *yices-quant-expand-limit*)
		       (let ((translate-rst-list
			     (loop for i from 0 to bnd
				   collect
				   (translate-to-yices2-forall rst expression
							       (cons (cons fst i) bindings)))))
			(format nil "(and ~{ ~a~})" translate-rst-list))))
		 (t (translate-to-yices-binding-expr (mk-forall-expr expr-bindings expression) bindings)))))
	 (t (translate-to-yices2* expression bindings))))

(defun translate-to-yices2-exists (expr-bindings expression bindings)
  (cond (expr-bindings
	 (let* ((fst (car expr-bindings))
		(fsttype (type fst))
		(below (simple-below? fsttype))
	   ...
	   (cond (bnd
		  (and (< bnd *yices-quant-expand-limit*)
		       (let ((translate-rst-list
			      (loop for i from 0 to bnd
				    collect
				    (translate-to-yices2-exists rst expression
								(cons (cons fst i) bindings)))))
...
(defmethod translate-to-yices2* ((expr forall-expr) bindings)
  (with-slots ((ebindings bindings) expression) expr
    (let ((result (translate-to-yices2-forall ebindings expression bindings)))
      (or result
	  (call-next-method)))))


(defmethod translate-to-yices2* ((expr exists-expr) bindings)
  (with-slots ((ebindings bindings) expression) expr
    (or (translate-to-yices2-exists ebindings expression bindings)
	(call-next-method))))
