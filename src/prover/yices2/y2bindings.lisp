...
;; %%%%%%%%%%% Setup/Parameters/Error Conditions

(define-condition y2api-error (simple-error) (error-string))
(define-condition y2api-warning (simple-warning) (warning-string))

(defmacro y2api-err (ctl &rest args)
  `(error 'y2api-error
	  :format-control ,ctl
	  :format-arguments (list ,@args)))

(defmacro y2api-warn (ctl &rest args)
  `(warn 'y2api-warning
	 :format-control ,ctl
	 :format-arguments (list ,@args)))

(defvar +y2api/dylib-loaded?+ nil)
(defvar +y2api/mcsat-enabled?+ nil)

(defvar +y2api/build-arch+ nil)
(defvar +y2api/build-mode+ nil)
(defvar +y2api/build-date+ nil)

(defvar +y2api/version-string+ nil)
(defvar +y2api/version-major+ nil)
(defvar +y2api/version-minor+ nil)
(defvar +y2api/version-patch+ nil)
...
(defun y2api/load-yices-dylib ()  
  (if *y2api/yices2-dylib-path*
      (progn
	(format t "y2api // dylib @ ~a~%" *y2api/yices2-dylib-path*)
	(setf *y2api/yices2-dylib-path*
	      (namestring
	       (truename *y2api/yices2-dylib-path*)))
	(eval `(cffi:define-foreign-library yices2
		   (t ,*y2api/yices2-dylib-path*)))
	(setq +y2api/dylib-loaded?+ t)
	(cffi:use-foreign-library yices2)
	(format t "y2api // ok. dylib loaded.~%"))
      (y2api-err "Yices2 dylib not found! Please set *y2api/yices2-dylib-path* in your .pvs.lisp!")))

(y2api/load-yices-dylib)

(cffi:defcvar ("yices_version" %y2api/version-string :library yices2) :string)
(cffi:defcfun ("yices_has_mcsat" %y2api/yices-has-mcsat :library yices2) :int32)
(cffi:defcvar ("yices_build_arch" %y2api/build-arch) :string)
(cffi:defcvar ("yices_build_mode" %y2api/build-mode) :string)
(cffi:defcvar ("yices_build_date" %y2api/build-date) :string)

(defun y2api/check-dylib-and-set-vars ()
  (when +y2api/dylib-loaded?+
    (let* ((version %y2api/version-string)
	   (parts (and version
		       (uiop:split-string version :separator '(#\.))))
	   (major (first parts))
	   (minor (second parts))
	   (patch (third parts)))
      (setq +y2api/version-string+ version)
      (setq +y2api/version-major+ major
	    +y2api/version-minor+ minor
	    +y2api/version-patch+ patch)
      (setq +y2api/build-arch+ %y2api/build-arch
	    +y2api/build-mode+ %y2api/build-mode
	    +y2api/build-date+ %y2api/build-date)
      (setq +y2api/mcsat-enabled?+
	    (not (zerop (%y2api/yices-has-mcsat))))
      (format t "y2api // mcsat? ~:[no~;yes~]~%" +y2api/mcsat-enabled?+)
      (format t "y2api // version: ~a~%" +y2api/version-string+)
      (format t "y2api // build-arch: ~a~%" +y2api/build-arch+)
      (format t "y2api // build-mode: ~a~%" +y2api/build-mode+)
      (format t "y2api // build-date: ~a~%" +y2api/build-date+)
      (format t "y2api // ok - y2api check complete.~%"))))

(y2api/check-dylib-and-set-vars)

;; %%%%%%%%%%% Constants/Terms

(defconstant +y2/null-term+ -1)
(defconstant +y2/null-type+ -1)
...
;; %%%%%%%%%%% API CFFI macro generator (from api.spec)
...
(eval-when (:compile-toplevel :load-toplevel :execute)
  ...
  (defun yices2-defcfun-form (declaration)
    (destructuring-bind (name return-type params)
      ...
      (let* ((y2api/cfun-name
	      (concatenate 'string "%Y2/" (string-upcase name))))
	(format t "y2api // defining ~a || ~a.~%" y2api/cfun-name name)
	`(cffi:defcfun (,(intern y2api/cfun-name :pvs) ,name)
