...
;*        PVSEMACS - set if the pvs lisp image was invoked from emacs
;*        PVS_LIBRARY_PATH - 
...
(defun pvs-init (&optional dont-load-patches dont-load-user-lisp path)
  ...
  (setq *pvs-emacs-interface*
	(let ((ei (environment-variable "PVSEMACS")))
	  (and ei (not (equal ei "")))))
  (setq *noninteractive*
  ...
  ;; If port is set, start the Websocket server
  ...
  (when (string= (subseq top-level:*prompt* 0 2) "~&")
    ;; (setf (char top-level:*prompt* 1) #\%)  ; Causes store into purespace address error
    (setq top-level:*prompt* (concatenate 'string "~%" (subseq top-level:*prompt* 2))))
  ;; #+sbcl
  ;; (when (find-package :sb-aclrepl)
  ;;   (let ((*package* (find-package :sb-aclrepl)))
  ;;     (when (and (boundp '*prompt*)
  ;; 		 (not (string= (subseq *prompt* 0 2) "~%")))
...
(defun find-pvs-path-from (dir)
  (cond ((pathname-equal dir "/") nil)
	((possible-pvs-path dir))
	(t (let ((pdir (namestring
			(asdf/pathname:pathname-parent-directory-pathname dir))))
	     (find-pvs-path-from pdir)))))

(defun initial-pvs-path (&optional path)
  (cond (path
	 (if (possible-pvs-path path)
	     path
  ...
  (let ((libs nil)
	...
	(dolist (dir dirs)
	  ...
	  (if (uiop:directory-exists-p dir)
	      ...
	      (pvs-message "Directory ~a in PVS_LIBRARY_PATH does not exist" dir)))))
...
(defun get-duplicate-libraries ()
  (let ((lib-alist nil))
    (dolist (lib (pvs-library-alist))
      (let ((entry (assq (car lib) lib-alist)))
	(if entry
	    (setf (cdr entry) (append (cdr entry) (list (cdr lib))))
	    (push (cons (car lib) (list (cdr lib))) lib-alist))))
    (remove-if-not #'(lambda (entry) (cddr entry)) lib-alist)))

(defun library-path-to-id (abspath)
  (car (rassoc abspath (pvs-library-alist) :test #'file-equal)))

(defun library-id-to-path (lib-id)
  (cdr (assoc lib-id (pvs-library-alist) :test #'string=)))
	   ...
	   (and spec-date
		...
		(every #'(lambda (te)
			   (let ((bin-date (file-write-date
				 ...
				 (expected-bin-date
				  (cdr (assoc (te-id te) bin-dates :test #'string=))))
			     (and bin-date
				  expected-bin-date
...
(defun get-pvs-file-comments (fileref)
  (with-pvs-file (filename) fileref
    (mapcar #'(lambda (th) (cons (id th) (get-theory-comments th)))
...
(defvar *collected-comments* nil)

(defun collect-all-newline-comments (dir &optional out-file)
  "Writes a JSON file of the form
[{\"pvs-file\": file, \"theories\": [{\"theory\": id, \"comments\": [ comment, ... ]
where each comment has the form
[comment-string, [start-row, start-col, end-row, end-col]]"
  (let ((dirpath (uiop:directory-exists-p dir)))
    (unless dirpath (error "~s is not a directory" dir))
    (when (and out-file
	       (not (directory-p (directory-namestring outfile))))
      (error "Output file directory for ~s does not exist" out-file))
    (let ((outf (or out-file
		    (merge-pathnames "comments.json" dirpath))))
      (collect-all-newline-comments* dirpath)
      (with-open-file (stream outf :direction :output :if-exists :supersede)
	(write (reverse (json:encode-json-to-string *collected-comments*)) :stream stream))
      (format t "~%Wrote file ~a" outf))))

(defun collect-all-newline-comments* (dir)
  (let ((pvs-files (uiop:directory-files dir "*.pvs"))
	...
	(let* ((file (pvs-filename path))
	       (theories (or (get-theories file)
				 ...
				 (parse-file file nil t)
			       (error (c)
				 (format t "~%ERROR in file ~a: ~a" path c) nil))))
	       (th-alist (mapcar #'(lambda (th)
				     `(("theory" . ,(string (id th)))
				       ("comments" . ,(newline-comments th))))
			   theories))
	       (file-cmts
		`(("pvs-file" . ,(namestring path))
		  ("theories" . ,th-alist))))
	  (push file-cmts *collected-comments*))))
    (dolist (subdir subdirs)
      (collect-all-newline-comments* subdir))))
...
(defun check-for-context-theory-clashes (new-theories filename forced?)
  (unless *generating-adt*
    (let ((clashes (collect-theory-clashes new-theories (pvs-filename filename))))
      (when clashes
...
(defmethod all-importings ((theories list))
  (if (null theories)
	  ...
	  (let ((*current-context* (or *current-context*
	    ...
	    (assert (or *current-context* (not (typechecked? (car theories)))))
...
(defun theory-section-of (decl theory)
  ...
  (cond ((memq decl (formals theory)) 'formals)
	((memq decl (theory-formal-decls theory)) 'theory-formal-decls)
	((memq decl (assuming theory)) 'assuming)
  ...
  (let ((osec (when last-kept-decl (theory-section-of last-kept-decl othy)))
	...
	(cond ((eq cursec osec)
		 ...
		 (setf (slot-value othy cursec) newsec)))
	      ...
	      (found-diff
	       ;; From now on, just copy new to old
...
Needs to pay attention to sections."
  ...
  (when (module? othy)
    (dolist (ty-decl (nonempty-types othy))
      (setf (nonempty? (car ty-decl)) nil)))
  (when (typep othy 'recursive-type)
    (let ((gen (make-specpath (id (adt-theory othy)))))
      (when
	  (uiop:file-exists-p gen)
	  (ignore-errors
...
(defun update-context-formula-proof-info (filename file nthy othy diff)
  ...
  ;; nil = no diff
  (declare (ignore othy diff))
  (let ((ce (get-context-file-entry filename)))
    (unless (or (null ce)
		(and (ce-write-date ce)
		     (= (file-write-time file)
			(ce-write-date ce))))
      
	(dolist (te (ce-theories ce))
	  (let ((lib-deps (if (pathname-equal (context-path nthy) (current-context-path))
			      (assq nil (te-dependencies te))
			      (assoc (context-path nthy) (te-dependencies te)
				     :test #'pathname-equal))))
	    (when (and (member (string (id nthy)) (cdr lib-deps) :test #'string=)
		       (not (get-theory (te-id te))))
	      (invalidate-theory-proofs te)))))))

(defun untypecheck-usedbys (theory)
  ;;(format t "~%Untypechecking ~{~%  ~a~}" (cons (id theory) (find-all-usedbys theory)))
...
(defun get-tccs (thref)
  (let* ((theory (get-typechecked-theory thref))
    ...
    (mapcar #'(lambda (tcc)
		`(("id" . ,(string (id tcc)))
		  ("theory" . ,(string (id (module tcc))))
		  ("comment" . ,(newline-comment tcc))
		  ("from-decl" . ,(string (get-unique-id (generated-by tcc))))
		  ("definition" . ,(str (definition tcc)))
		  ("proved" . ,(if (proved? tcc) "true" "false"))))
      tccs)))
  ...
  (unless (and (default-proof decl)
	...
	(real-proof-time decl))
      ...
      (cond ((eq (status-flag proof) '!)
	     ...
	     t)
	    (t (setf (proof-status decl) 'unfinished)
...
(defun sizeof-proofs-pvs-file (filename)
  (let ((theories (cdr (gethash (pvs-filename filename) (current-pvs-files)))))
    (if theories
	(pvs-buffer "Proof Sizes"
	  (with-output-to-string (out)
	    (let ((total 0))
...
(defun sizeof-proofs-proofchain-at (filename declname line
					     &optional (origin "pvs"))
  (if (or (gethash (pvs-filename filename) (current-pvs-files))
...
(defun declname-to-decl (declname theory)
  (let ((decl-and-pos (uiop:split-string declname :separator "-")))
...
(defmethod parsed?* ((mod datatype-or-module))
  (or (memq mod *parsed-theories-seen*)
  ...
  (if libref
	...
	(and lib-path
	     (let* ((ws (get-workspace-session lib-path)))
	       (and ws
		    (cdr (gethash (pvs-filename filename) (pvs-files ws)))))))
      (let ((fn (pvs-filename filename)))
...
;; (with-theory theory-ref &rest body)
...
nil is returned in that case."
  (let* ((mod (get-theory theoryref))
	 (fname (when (and mod (filename mod))
...
;; (defun collect-library-theories ()
;;   (mapcan #'(lambda (elt)
;; 	      (mapcar #'(lambda (fpath) (scan-for-theories fpath))
;; 		(uiop:directory-files (cdr elt) "*.pvs")))
;;     (pvs-library-alist)))
...
(defun parsed-date (filename)
  (car (gethash (pvs-filename filename) (current-pvs-files))))
...
(defun show-strategy (strat-name)
  (let* ((strat-id (intern #+allegro (string-downcase strat-name)
    ...
    (if strategy
	;; name contents &optional display? read-only? append? kind
...
(defun collect-rules-used* (sexp &optional bindings)
  (cond ((null sexp) nil)
	...
	((backquoted? sexp)
	 (collect-rules-used* (cadr sexp) bindings))
	((unbackquoted? sexp)
	 (break "unbackquoted?")
	 (cons (car sexp) (collect-rules-used* (cdr sexp) bindings)))
	((backquoted? (car sexp))
	 (collect-rules-used* (cadr (car sexp))))
	((symbolp (car sexp))
...
#+allegro
(defun fix-strategy-case-problems ()
  "If a strategy (e.g., dl-Vd) is read in SBCL, then saved in a .prf file,
it becomes lower-case for Allegro, and then not found. This function creates
lower-case versions for these, complaining if it could result in duplicates."
  (let ((names nil))
    (do-all-strategies #'(lambda (entry)
			   (when (some #'upper-case-p (string (name entry)))
			     (push (name entry) names))))
    (dolist (name names)
      (let ((lname (intern (string-downcase (string name)) :pvs)))
	(cond ((let ((entry (gethash name *rulebase*)))
		 (when entry
		   (setf (gethash lname *rulebase*) entry))))
	      ((let ((entry (gethash name *rules*)))
		 (when entry
		   (setf (gethash lname *rules*) entry))))
	      ((let ((entry (gethash name *steps*)))
		 (when entry
		   (setf (gethash lname *steps*) entry)))))
	(let ((keywords (assq name *prover-keywords*)))
...
(defun make-proof-cmds-orgtbl ()
  (let ((cmds nil))
    (maphash #'(lambda (id step) (declare (ignore step)) (push id cmds)) *rulebase*)
    (maphash #'(lambda (id step) (declare (ignore step)) (push id cmds)) *rules*)
    (format t "~{~%| ~a | | | |~}" (sort cmds #'string<))))
...
(defvar *help-topics* nil
  "An alist of topics, car is a string, cdr is a string or another help-topic alist")

(defun valid-help-contents (contents)
  (or (stringp contents)
      (and (consp contents)
	   (every #'(lambda (cnt) (and (consp cnt)
				       (stringp (car cnt))
				       (valid-help-contents (cdr cnt))))
		  contents))))

(defun defhelp (topic contents)
  (assert (stringp topic))
  (assert (valid-help-contents contents))
  (let ((cur-topic (assoc topic *help-topics* :test #'string=)))
    (if (null cur-topic)
	(push (cons topic contents) *help-topics*)
...
;; *debugger-hook* should be bound when in the prover/PVSio
;; Can give debugging hints
(defvar pvs-prover-help-string ()
  "Prover interrupted: type
(restore) - continues from previous proofstate
*proof-strategy-stack* - shows the stack of prover commands
*ps* - the current proofstate")
