      ...
      (or (bound-variable-resolution resolutions)
		 ...
		 (unpreses (or (and (cdr ureses)
				    (remove-if #'(lambda (res)
				      ...
				      ureses))
			       ureses))
		 (nmap-reses (or (and (cdr unpreses)
				      (remove-if #'(lambda (res)
						     (mapped-const-decl? (declaration res)))
					unpreses))
				 unpreses))
		 (last-reses (or (and (cdr nmap-reses)
				      (mod-id ex)
  ...
  (when (mappings thinst)
    (let* ((*tccs-generated-for* (when (boundp '*tccs-generated-for*)
      ...
      (let* ((smappings (sort-mappings (mappings thinst)))
             ...
             (there? (and (fully-typed? thinst)
			  (member thinst entry :test #'tc-eq)))
...
(defun set-type-mapping-rhs*-theoryref (thid lhs rhs thinst mapping)
  ...
  (let ((threses (remove-if
    ...
    (cond ((cdr threses)
          ...
          ((null threses)
           (type-error (expr rhs) "~a is not an instance of ~a" (expr rhs) thid))
	...
	;; coptypes are funtypes whose range is compatible with expected
        (let* ((optypes1 (if (cdr coptypes)
               ...
               (optypes6 (if (cdr optypes5)
                             (or (explicit-importings operator optypes5)
                                 optypes5)
                             optypes5))
