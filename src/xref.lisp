...
(defmethod regenerate-xref ((use importing))
...
(defmethod generate-xref ((use exporting))
  nil)
...
(defmethod from-prelude-library? ((th datatype-or-module))
  (member (context-path th) (current-prelude-libraries)
	 :test #'file-equal))
...
(defmethod visible-theory? ((th datatype-or-module))
  (assert *current-context*)
  (or (from-prelude? th)
      (from-prelude-library? th)
      (memq th (all-importings (current-theory)))))

(defmethod visible-theory? ((id symbol))
  (assert *current-context*)
  (or (eq id (id (current-theory)))
      (gethash id *prelude*)
      (let ((found nil))
	(dolist (lib-path (current-prelude-libraries))
	  (unless found
	    (let ((ws (get-workspace-session lib-path)))
	      (when (gethash id (pvs-theories ws))
		(setq found t)))))
	found)
      (some #'(lambda (th)
		(or (eq id (id th))
		    (and (recursive-type? th)
			 (or (eq id (id (adt-theory th)))
			     (and (adt-map-theory th)
				  (eq id (id (adt-map-theory th))))
			     (and (adt-reduce-theory th)
				  (eq id (id (adt-reduce-theory th))))))))
	    (let* ((adecls (all-decls (current-theory)))
		   ...
		   (imps (mapcan #'(lambda (thname)
					...
					(or (and (typep th 'datatype)
					    ...
					    (list thname)))))
			   thnms)))
	      (assert (every #'resolution imps))
	      (when imps
		(all-importings* (current-theory) imps))))))
