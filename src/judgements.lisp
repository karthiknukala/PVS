...
(defmethod add-judgement-decl ((jdecl name-judgement) &optional quiet?)
  ...
  (let* ((jname (cond ((name-expr? (name jdecl))
		       (name jdecl))
		      ((let ((macro (from-macro (name jdecl))))
			 (when macro
			   (assert (name-expr? (from-macro (name jdecl))))
			   (setf (associated-macro jdecl) macro)
			   macro)))
...
(defun best-judgement-type (expr)
  (let ((jtypes (judgement-types+ expr)))
    (cond ((or (null jtypes) (vectorp jtypes)) (type expr))
	  ((and (cdr jtypes)
		(every #'(lambda (jty) (strict-compatible? (car jtypes) jty))
		       (cdr jtypes)))
	   (let* ((*generate-tccs* 'none)
		  (inter (ignore-errors (reduce #'intersection-type jtypes))))
	     ;;(unless (fully-instantiated? inter) (break "Not fully-instantiated?"))
	     (or inter
		 (car jtypes))))
	  (t (car jtypes)))))
	  ...
	  (multiple-value-bind (types jdecls thinsts)
	    ...
	    #+pvsdebug (assert (or (vectorp types)
				   (every #'(lambda (ty)
					  ...
					  types)))
	    (unless (lambda-expr-with-type? ex)
	   ...
	   (let ((*judgement-bound-vars* *judgement-bound-vars*)
	     ...
	     (unwind-protect
		  (let ((*bound-variables* (append (bindings (operator* ex))
						   *bound-variables*))
			(jex (get-operator*-expression ex)))
		    (unless (eq jex ex)
		 ...
		 (setf (gethash (car jbvar) (judgement-types-hash
					     (current-judgements)))
		       (cdr jbvar)))))
	 #+pvsdebug (assert (every #'(lambda (ty) (compatible? ty (type ex))) ljtypes))
	 (when ljtypes
...
(defun get-matching-application-judgement-types (ex)
  (let* ((op (operator* ex))
	...
	(multiple-value-bind (bindings ftype)
	  ...
	  (when bindings
	    (let ((thinsts (create-compatible-modinsts thname decl (list bindings) nil)))
  ...
  (let ((sty (find-supertype ftype)))
    (when (funtype? sty)
      (let* ((jargstypes ;;(mapcar #'judgement-types+ (car args))
	      (mapcar #'(lambda (arg) (reduce #'compatible-type (judgement-types+ arg)))
		(car args)))
	...
	(if (null (cdr args))
	    ...
	    (let* ((nfran nil)
		   (nbdgs (multiple-value-bind (nbds nty)
			      (get-matching-application-judgement-types*
			       (cdr args) fran nbindings)
...
(defun get-operator*-expression (ex)
  (if (and (application? ex)
	   (lambda-expr? (operator ex)))
      (get-operator*-expression (expression (operator ex)))
      ex))
...
;; (defmethod judgement-types* ((ex list-expr))
;;   (let* ((top-type (find-declared-adt-supertype (type ex)))
;; 	 (type-value (car (actuals top-type)))
;; 	 (car-jtype (reduce #'compatible-type (judgement-types* (args1 ex))))
;; 	 (jtype (judgement-list-types (args2 ex) car-jtype top-type)))
;;     ;; Need to substitute; not trivial
;;     (break "list-expr")))

;; (defun judgement-list-types (list-ex ctype top-type)
;;   (if (null-expr? list-ex)
;;       (list ctype)
;;       (let ((nctype (reduce #'compatible-type (judgement-types* (args1 list-ex))
;; 			    :initial-value ctype)))
;; 	(unless (tc-eq ctype top-type)
;; 	  (judgement-list-types (args2 list-ex) nctype top-type)))))
  ...
  (if (null gen-judgements)
	...
	(if jtype
		...
		(let* ((dont-add? (or (null jrange)
				      ...
				      (some #'(lambda (jty) (subtype-of? jty jrange))
					    jtypes)))
		       (rtypes nil)
		       (rdecls nil))
		  (if dont-add?
...
(defun instantiate-generic-appl-judgement-type (ex judgement)
  (let* ((op (operator* ex))
	 (jfmls (formals judgement))
	 (bindings1 (tc-match op (name judgement)
			      (mapcar #'list
				(formals-sans-usings (module judgement)))))
	 (bindings (when bindings1
			 ...
			 (tc-match (car (judgement-types+ (argument ex)))
				   (if (cdr (car jfmls))
				       (mk-tupletype (mapcar #'type (car jfmls)))
				       (type (caar jfmls)))
				   bindings1)))))
	...
	(let ((stype (subst-mod-params (judgement-type judgement) jthinst th)))
	  stype)))))
    ...
    (let* ((depbdg (cond ((dep-binding? (domain (type ex)))
      ...
      (let ((ftypes (mapcar #'(lambda (rty)
				(let ((dom (if (member depbdg (freevars rty)
				  ...
				  (mk-funtype dom rty)))
		      rtypes)))
	#+pvsdebug (assert (every #'(lambda (fty) (compatible? fty (type ex))) ftypes))
	#+pvsdebug (assert (every #'(lambda (fv)
...
(defmethod judgement-types* ((ex lambda-expr-with-type))
  (multiple-value-bind (ejtypes ejdecls)
      (call-next-method)
    (values ejtypes ejdecls)))
...
(defmethod subst-params-decl ((j name-judgement) thname theory)
  (let ((jname (cond ((name-expr? (name j))
		      (name j))
		     ((associated-macro j)
		      (assert (name-expr? (associated-macro j)))
		      (associated-macro j))
		     (t (error "Bad name-judgement ~a" j)))))
    (if (or (memq theory (free-params-theories j))
	    (and (mappings thname)
		 (memq (declaration jname) (all-decls theory))))
	(let* ((smphash (cdr (get-subst-mod-params-caches thname)))
	       (hj (gethash j smphash)))
	  (if (and hj
	      ...
	      (let* ((nj (lcopy j
			   ...
			   'declared-type (subst-mod-params (declared-type j)
					      thname theory)
			   'name (subst-mod-params jname thname theory)))
		     (oj (car (member nj
				      (remove-if-not
					  #'(lambda (d)
					      (eq (module d) (current-theory)))
...
(defmethod type-constraints* ((te funtype) ex preds all?)
  (if (memq all? '(:funcs t))
      (let ((npreds (mapcar #'(lambda (tp)
				(make!-application tp ex))
		      (type-predicates te all?))))
