...
(defmethod typecheck* ((db dep-binding) expected kind arguments)
  ...
  (unless (type db)
    (let* ((*generate-tccs* 'none)
	   (dtype (typecheck* (declared-type db) nil nil nil)))
      (setf (type db) dtype))
    (set-type (declared-type db) nil)
    (unless (or (null (print-type (type db)))
		(tc-eq (print-type (type db)) (declared-type db)))
      (let* ((te (declared-type db))
	     (ptype (typecase te
		      (type-name
		       (let ((decl (declaration te))
			     (mi (module-instance te)))
			 (change-class (copy te) 'print-type-name
			   :actuals (when (actuals te) (actuals mi))
			   :dactuals (when (dactuals te) (dactuals mi))
			   :resolutions (list (mk-resolution decl mi nil)))))
		      (type-application (change-class (copy te)
					    'print-type-application))
		      (expr-as-type (change-class (copy te)
					'print-expr-as-type)))))
	(setf (print-type (type db)) ptype))))
  db)
