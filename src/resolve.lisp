...
(defmethod typecheck* ((name name) expected kind argument)
  ...
  (let* ((*resolve-name* name)
	 ...
	 (args (if (and argument (eq k 'expr))
		   ...
		   argument))
	 (nres (resolve* name k args)))
    (set-actuals-places name nres)
    (when (and (cdr nres)
	       (resolution name)
	       (member (resolution name) nres :test #'tc-eq))
      (setf nres (copy-list (resolutions name))))
    ...
    (when (and (null nres)
      ...
      (setq nres (let ((ares (append (argument-conversion name args)
    ...
    (cond ((null nres)
	  ...
	  ((and (cdr nres)
		(not (eq k 'expr)))
	   (let* ((nnres (if argument
			    (delete-if-not #'(lambda (r)
					       (formals (declaration r)))
			      nres)
			    nres))
		  (fres (or (delete-if-not #'fully-instantiated? nres) nnres))
	   ...
	   (let* ((*resolve-name* name)
	     ...
	     (cond ((cdr res)
		   ...
		   (t (let ((theory (declaration (car res))))
			...
			(when (mappings name)
			  (typecheck-mappings (mappings name) name))
			(unless (and (fully-typed? name)
				     (member name (get-importings theory) :test #'tc-eq))
...
(defmethod compatible-arguments? (decl modinst args mod)
  ...
  (if (null args)
	...
	(and dtypes
		 ...
		 (compatible-uninstantiated? decl modinst dtypes dargs)
		 (let* ((*generate-tccs* 'none)
			(*smp-include-actuals* t)
			(*smp-dont-cache* t)
			(domtypes (subst-mod-params dtypes modinst (module decl)
						    (if (decl-formal? decl)
							(current-declaration)
							decl))))
		   (assert (fully-instantiated? domtypes))
		   ;;(set-type-actuals-and-maps modinst)
		   (when (compatible-args? decl dargs domtypes)
		     (list modinst))))))))
  ...
  (or (null args)
      (cond ((and (injection-application? (car args))
		 ...
		 (pushnew (find-supertype (car types)) (types (car args))
			  :test #'tc-eq)
		 t)))
	    ((and (name-expr? (car args))
		  (some #'(lambda (res)
			    (and (memq (declaration res) *bound-variables*)
				 (compatible? (type res) (car types))))
