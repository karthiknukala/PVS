...
other 64-bit platforms using Virtualbox or VMWare.  PVS is implemented in
Common Lisp\index{Common Lisp}, but it is not necessary to know Lisp to
effectively use the system.\footnote{An exception to this is in writing
  complex prover strategies.}

PVS was developed with the Emacs (\gnuemacs\index{Gnu Emacs} editor, which
also acts as a GUI. In addition, NASA has provided a VSCode interface to
PVS as an alternative (\url{http://github.com/nasa/vscode-pvs}). This
guide will focus on the Emacs interface.

familiarity with Emacs and access to
the GNU Emacs manual~\cite{emacs20} (usually available as an info file)
are desirable.
...
PVS specifications are {\sc UTF-8} files prepared and
