...
(eval-when-compile
  ...
  (defconst pvs-ltx--mark-re (regexp-opt (mapcar #'car pvs-ltx--mark-map)))

  (defun pvs-ltx--ascii-p (char)
    (and (characterp char) (< char 128)))

  (defmacro pvs-ltx--define-rules (&rest rules)
    (load "uni-name" nil t)
    (let ((newrules ()))
      (dolist (rule rules)
        (pcase rule
		 ...
		 (dolist (pair (ucs-names))
		   (let ((name (car pair))
			 ...
			 (if (listp keys)
			     ...
			     (setq count (1+ count))
			     (push (list keys char) newrules))))))
		 ;; (message "pvs-ltx: %d mappings for %S" count re)
		 )))))
...
(pvs-ltx--define-rules
 ...
 ((lambda (name char)
    (let* ((c (if (match-end 1)
                ...
                (match-string 2 name)))
           (mark1 (cdr (assoc (match-string 3 name) pvs-ltx--mark-map)))
           (mark2 (if (match-end 4)
                      (cdr (assoc (match-string 4 name) pvs-ltx--mark-map))))
 ...
 ((lambda (name char)
    (let* ((mark (cdr (assoc (match-string 1 name) pvs-ltx--mark-map))))
      (cl-assert mark)
      (list (format "\\%s" mark))))
  (concat "\\`COMBINING \\(" pvs-ltx--mark-re "\\)\\(?: ACCENT\\)?\\'"))

 ((lambda (name char)
    (unless (pvs-ltx--ascii-p char)
      (let* ((mark (cdr (assoc (match-string 1 name) pvs-ltx--mark-map))))
        (cl-assert mark)
        (list (format "\\%s{}" mark)))))
  (concat "\\`\\(?:SPACING \\)?\\(" pvs-ltx--mark-re "\\)\\(?: ACCENT\\)?\\'"))
 ...
 ((lambda (name char)
    (unless (or (pvs-ltx--ascii-p char)
