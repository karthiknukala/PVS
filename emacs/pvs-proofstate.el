...
(defun make-proofstate-frame (proofstate)
  (let* ((name (format "%s" (cdr (assoc 'id proofstate))))
	 (ps-buf (get-buffer-create name))
	 (seq (assq 'sequent proofstate)))
    (with-current-buffer ps-buf
      (insert-proofstate-sequent seq)
...
;; (define-derived-mode pvs-proof-session-mode comint-mode "PVS Proof Session"
;; 		     "Major mode for interacting with a proof session
;; The following commands are available:
;; \\{pvs-proof-session-mode-map}")

(defvar pvs-proof-session-mode-map
  (let ((map (nconc (make-sparse-keymap) comint-mode-map)))
    ;; example definition
    ;;(define-key map "\t" 'completion-at-point)
    map)
  "Basic mode map for `proof-sessions'.")
